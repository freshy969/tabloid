...
/*
	A useful general-purpose 'sort by' function
*/
...
define('ARRAY_WITH_FIRST', null); // collect the elements together in the position of the first one found
define('ARRAY_WITH_LAST', 0.6); // collect the elements together in the position of the last one found
define('ARRAY_AT_START', 0.1); // place all the elements at the start of the array
define('ARRAY_AT_END', 0.9); // place all the elements at the end of the array
 ...
 * end of the array). Any of theARRAY_* values defined above can also be passed in the $beforekey parameter.
...
function qa_array_reorder(&$array, $keys, $beforekey = null, $reorderrelative = true)
{
	...
	foreach ($array as $key => $value) {
		...
		if (isset($keyorder[$key])) {
			...
			if ($beforekey ==ARRAY_WITH_LAST || ($beforekey ===ARRAY_WITH_FIRST && !isset($offset)))
				$offset = count($newkeys);
	...
	if (!isset($offset)) // also good forARRAY_AT_END
		$offset = count($newkeys);
