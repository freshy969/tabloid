...
/*
	Some useful string-related stuff
*/
...
/* Speed up indexing : Remove Japan/Korean symbols processing and do not treat apostrophes with respect
...
*/
	...
	$parts = preg_split('/(' . $separator . '+)/', $string, -1, PREG_SPLIT_NO_EMPTY | ($delimiters ? PREG_SPLIT_DELIM_CAPTURE : 0));

	$words = [];
	foreach ($parts as $word) 
		if (mb_strlen($word) >= 3) // "word" length should be at least 3 letters
			$words[] = $word;
...
function qa_block_words_explode($wordstring)
{
	...
	return preg_split('/' .PREG_BLOCK_WORD_SEPARATOR . '+/', $wordstring, -1, PREG_SPLIT_NO_EMPTY);
}
...
function qa_block_words_to_preg($wordsstring)
{
	...
	foreach ($blockwords as $blockword) { // * in rule maps to [^ ]* in regular expression
		...
		if (!preg_match('/^(' .PREG_CJK_IDEOGRAPHS_UTF8 . ')/', $blockword))
...
function qa_block_words_match_all($string, $wordspreg)
{
	...
	if (strlen($wordspreg)) {
		...
		$string = preg_replace('/' .PREG_BLOCK_WORD_SEPARATOR . '/', ' ', $string);
