...
/*
	External user functions for basic Joomla integration
*/
...
function qa_get_logged_in_user()
{
	...
	if ($user && !$user->guest) {
		...
		$level = USER_LEVEL_BASIC;

		if ($access['post']) {
			$level = USER_LEVEL_APPROVED;
		}
		if ($access['edit']) {
			$level = USER_LEVEL_EDITOR;
		}
		if ($access['mod']) {
			$level = USER_LEVEL_MODERATOR;
		}
		if ($access['admin']) {
			$level = USER_LEVEL_ADMIN;
		}
		if ($access['super'] || $user->get('isRoot')) {
			$level = USER_LEVEL_SUPER;
		}
...
class qa_joomla_helper
{
	...
	private function find_joomla_path()
	{
		...
		if (!defined('JPATH_BASE')) {
			define('JPATH_BASE',FINAL_JOOMLA_INTEGRATE_PATH);
		}
	}
