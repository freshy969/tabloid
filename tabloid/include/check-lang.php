...
/*
	Development tool to see which language phrases are missing or unused
*/

define('BASE_DIR', dirname(dirname(empty($_SERVER['SCRIPT_FILENAME']) ? __FILE__ : $_SERVER['SCRIPT_FILENAME'])) . '/');
...
<html>
	<head>
		...
		<title>Tabloid Language Check</title>
		<style>
			body {
				font-family: arial;
				font-size: 12px;
...
echo '<h1>Checking US English files in <code>include</code>...</h1>';

$includefiles = array_merge(glob(INCLUDE_DIR . '*.php'), glob(INCLUDE_DIR . '*/*.php'), glob(PLUGINS_DIR . '*/*.php'));
...
foreach ($includefiles as $includefile) {
	...
	if (preg_match('|/include/langs-([a-z]+)\.php$|', $includefile, $matches)) { // it's a lang file
		$prefix = $matches[1];
...
foreach ($languages as $code => $language) {
	echo '<h1>Checking ' . $language . ' files in <code>langs/' . $code . '</code>...</h1>';

	$langdefined = array();
	$langdifferent = array();
	$langsubstitutions = array();
	$langincludefiles = glob(LANGS_DIR . $code . '/qa-*.php');
	$langnewphrases = array();

	output_start_includes();

	foreach ($langincludefiles as $langincludefile) {
		if (preg_match('/lang-([a-z]+)\.php$/', $langincludefile, $matches)) { // it's a lang file
			$prefix = $matches[1];
	...
	foreach ($defined as $key => $valuecount) {
		$showaserror = !(($key == 'admin') || ($key == 'options') || ($code == 'en'));
	...
	foreach ($langnewphrases as $prefix => $phrases) {
		echo '<h2>' . $language . ' phrases to add to <code>langs/' . $code . '/lang-' . $prefix . '.php</code>:</h2>';

		echo 'Copy and paste this into the middle of <code>langs/' . $code . '/lang-' . $prefix . '.php</code> then translate the right-hand side after the <code>=></code> symbol.';
...
function output_lang_issue($prefix, $key, $issue, $error = true)
{
	...
	echo sprintf(
		'<code class="%s">lang-<strong>%s</strong>.php:%s</code> &nbsp; %s<br>',
