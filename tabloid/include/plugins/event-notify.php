...
/*
	Event module for sending notification emails
*/
...
class event_notify
{
	...
	{
		require_once INCLUDE_DIR . 'app/emails.php';
		...
		switch ($event) {
			case 'q_post':
				...
				if (isset($followanswer['notify']) && !qa_post_is_by_user($followanswer, $userid, $cookieid)) {
					...
					send_notification($followanswer['userid'], $followanswer['notify'], @$followanswer['handle'], qa_lang('emails/a_followed_subject'), qa_lang('emails/a_followed_body'), array(
						'^q_handle' => $sendhandle,
				...
				if (qa_opt('notify_admin_q_post'))
					send_notification(null, qa_opt('feedback_email'), null, qa_lang('emails/q_posted_subject'), qa_lang('emails/q_posted_body'), array(
						'^q_handle' => $sendhandle,
			...
			case 'a_post':
				...
				if (isset($question['notify']) && !qa_post_is_by_user($question, $userid, $cookieid))
					send_notification($question['userid'], $question['notify'], @$question['handle'], qa_lang('emails/q_answered_subject'), qa_lang('emails/q_answered_body'), array(
			...
			case 'c_post':
				...
				if (isset($parent['notify']) && !qa_post_is_by_user($parent, $userid, $cookieid)) {
					...
					if (email_validate($sendemail))
						$senttoemail[$sendemail] = true;
					elseif (isset($senduserid))
						$senttouserid[$senduserid] = true;

					send_notification($senduserid, $sendemail, @$parent['handle'], $subject, $body, array(
						'^c_handle' => $sendhandle,
				...
				foreach ($params['thread'] as $comment) {
					if (isset($comment['notify']) && !qa_post_is_by_user($comment, $userid, $cookieid)) {
						...
						send_notification($senduserid, $sendemail, @$comment['handle'], qa_lang('emails/c_commented_subject'), qa_lang('emails/c_commented_body'), array(
							'^c_handle' => $sendhandle,
							'^c_context' => $sendcontext,
							'^c_content' => $sendtext,
							'^url' => $sendurl,
			...
			case 'q_requeue':
				if (qa_opt('moderate_notify_admin')) {
					send_notification(null, qa_opt('feedback_email'), null,
			...
			case 'c_flag':
				...
				if ($notifycount >= 0 && ($notifycount % qa_opt('flagging_notify_every')) == 0) {
					send_notification(null, qa_opt('feedback_email'), null, qa_lang('emails/flagged_subject'), qa_lang('emails/flagged_body'), array(
			...
			case 'a_select':
				...
				if (isset($answer['notify']) && !qa_post_is_by_user($answer, $userid, $cookieid)) {
					...
					send_notification($answer['userid'], $answer['notify'], @$answer['handle'], qa_lang('emails/a_selected_subject'), qa_lang('emails/a_selected_body'), array(
			...
			case 'u_register':
				if (qa_opt('register_notify_admin')) {
					send_notification(null, qa_opt('feedback_email'), null, qa_lang('emails/u_registered_subject'),
			...
			case 'u_level':
				if ($params['level'] >= USER_LEVEL_APPROVED && $params['oldlevel'] < USER_LEVEL_APPROVED) {
					send_notification($params['userid'], null, $params['handle'], qa_lang('emails/u_approved_subject'), qa_lang('emails/u_approved_body'), array(
			...
			case 'u_wall_post':
				if ($userid != $params['userid']) {
					...
					send_notification($params['userid'], null, $params['handle'], qa_lang('emails/wall_post_subject'), qa_lang('emails/wall_post_body'), array(
