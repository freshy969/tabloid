...
/*
	Default theme class, broken into lots of little functions for easy overriding
*/
...
/*
	...
	that extends this base class html_theme_base. You can then override any of the methods below,
	referring back to the default method using double colon (html_theme_base::) notation.
...
class html_theme_base
{
	...
	public $params; // Yandex and Google
	...
	{
		//$attribution = '<!-- Powered by Tabloid - http://www.tabloid.dev/ -->';
    ...
    public function icons()
	{
        ...
        // HASHTAG = Font: Russo One + Size: 110
...
ICO;
	...
	{
		$YandexMetrics = <<<LONGSTRING1
<script>
   (function(m,e,t,r,i,k,a){m[i]=m[i]||function(){(m[i].a=m[i].a||[]).push(arguments)};
   m[i].l=1*new Date();k=e.createElement(t),a=e.getElementsByTagName(t)[0],k.async=1,k.src=r,a.parentNode.insertBefore(k,a)})
   (window, document, "script", "https://mc.yandex.ru/metrika/tag.js", "ym");
   ym(YANDEX_METRICS, "init", { clickmap:true, trackLinks:true, accurateTrackBounce:true });
</script>
<noscript><div><img src="https://mc.yandex.ru/watch/YANDEX_METRICS" style="position:absolute; left:-9999px;" alt="" /></div></noscript>
LONGSTRING1;

		$YandexMetrics = str_replace("YANDEX_METRICS", YANDEX_METRICS, $YandexMetrics);
		$this->output($YandexMetrics);

		$GoogleAnalytics = <<<LONGSTRING2
...
LONGSTRING2;
        ...
        $GoogleAnalytics = str_replace("GOOGLE_ANALYTICS", GOOGLE_ANALYTICS, $GoogleAnalytics);
...
MATHEXT;
	...
	public function page_title_error()
	{
		if (isset($this->content['title'])) {
            ...
            if (array_key_exists('q_view', $this->content) &&
                ($this->content['q_view']['raw']['type'] == 'POST' ||
                $this->content['q_view']['raw']['basetype'] == 'Q'))
            {
                $who = array_key_exists('who', $this->content['q_view']) ? $this->content['q_view']['who']['data'] : '';

                $views = $this->content['q_view']['raw']['views'];
                $date = array_key_exists('when', $this->content['q_view']) ? $this->content['q_view']['when']['data'] : '';
                $this->output('<div id="metas">');
                $this->output('<span class="icon-user">&nbsp;' . $who . '</span>&nbsp');
                $this->output('<span class="icon-time">&nbsp;' . $date . '</span>&nbsp');
                $this->output('<span class="icon-eye">&nbsp;' . $views . '</span>');
                $this->output('</div>');
            }
            ...
            if (array_key_exists('q_view', $this->content) && $this->content['q_view']['raw']['type'] == 'POST') {
			    $this->post_show_image();
                $this->post_show_lead();
            }
        ...
        // Banner Rotator
        if (rand(1,10)%2)
            $this->output('<!--<div style="margin-bottom: 5px;">--><a href="http://tabloid.dev" target="_blank"><img src="/banners/quora-banner.jpg" style="width: 100%; border-bottom: 1px solid #dee1e3; border-radius: 0px;" alt="Купить гитару в Москве"></a><!--</div>-->', '');
        else
            $this->output('<!--<div style="margin-bottom: 5px;">--><a href="http://tabloid.dev" target="_blank"><img src="/banners/reddit-banner.jpg" style="width: 100%; border-bottom: 1px solid #dee1e3; border-radius: 0px;" alt="Винтажный Клуб для гитаристов"></a><!--</div>-->', '');
    ...
    public function post_show_lead()
	{
        $lead = $this->content['q_view']['raw']['lead'];
        if (!$lead) return;

        $this->output('<div class="qa-q-item-lead">');
        $this->output('<h4>' . $lead . '</h4>');
        $this->output('</div>');
	}

    public function post_show_image()
	{
        $img = $this->content['q_view']['raw']['image'];
        if (!$img) return;

        $url = PROTOCOL . '://' . DOMAIN . '/' . $img;

        $this->output('<div class="qa-q-item-image">');
	...
	public function q_item_main($q_item)
	{
		...
		$this->item_title($q_item);

        if ($q_item['raw']['type'] == 'POST') {
            $this->show_lead($q_item);
		    $this->show_image($q_item);
		} 
		
        if ($q_item['raw']['type'] == 'Q') 
			$this->show_lead($q_item, 'small');			
    ...
    public function show_image($item)
	{
        if (!$item['raw']['image']) return;

        $this->output(
			'<div class="qa-q-item-image">',
            '<img src="' . PROTOCOL . '://' . DOMAIN . '/' . $item['raw']['image'] . '">',
			'</div>'
		);

    }

    public function show_lead($item, $style = null)
	{
        if (!$item['raw']['lead']) return;

		$this->output('<div class="qa-q-item-lead">');
		if ($style == 'small')
			$this->output('<p style="font-size: 0.85em; line-height: 1.4em;">' . $item['raw']['lead'] . '</p>');
		else	
			$this->output('<p>' . $item['raw']['lead'] . '</p>');
        $this->output('</div>');
	...
	public function item_title($item)
	{
		$this->output(
			...
			// add closed note in title
			empty($item['closed']['state']) ? '' : ' [' . $item['closed']['state'] . ']',
			'</div>'
		);
	}
	...
	// Duplicate code from format.php
	public function post_meta_when($post, $class)
	{
		if (!isset($post['raw']['created']))
			return;

		$now = new DateTime();
		$date = (new DateTime())->setTimestamp($post['raw']['created']);
		...
		$this->output_split([ 'data' => $date ], $class . '-when');
	}
