...
/*
	Sets up Q2A environment, plus many globally useful functions
*/

//const TABLOID_VERSION = '1.0.0';
//const BUILD_DATE = '2019-09-01';

const TABLOID_VERSION = '1.2.0';
const BUILD_DATE = '2020-01-05';
...
function qa_autoload($class)
{
	if (strpos($class, 'Q2A_') === 0)
		require INCLUDE_DIR . strtr($class, '_', '/') . '.php';
}
...
if (defined('WORDPRESS_LOAD_FILE')) {
	// if relevant, load WordPress integration in global scope
	require_onceWORDPRESS_LOAD_FILE;
} elseif (defined('JOOMLA_LOAD_FILE')) {
	// if relevant, load Joomla JConfig class into global scope
	require_onceJOOMLA_LOAD_FILE;
}
*/
...
if (!isset($autoconnect) || $autoconnect !== false) {
...
{
	return version_compare(TABLOID_VERSION, $version) < 0;
}
...
function qa_initialize_constants_1()
{
	...
	define('CATEGORY_DEPTH', 4); // you can't change this number!

	if (!defined('BASE_DIR'))
		die("[ERR] BASE_DIR is not defined!");

	define('EXTERNAL_DIR', BASE_DIR . 'external/');
	define('INCLUDE_DIR', BASE_DIR . 'include/');
	define('LANGS_DIR', BASE_DIR . 'langs/');
	define('THEMES_DIR', PUBLIC_DIR . 'themes/');
	define('PLUGINS_DIR', BASE_DIR . 'plugins/');

	if (!file_exists(BASE_DIR . 'config.php'))
		qa_fatal_error('[ERR] The config file could not be found!');

	require_once BASE_DIR . 'config.php';

	$qa_request_map = isset($CONST_PATH_MAP) && is_array($CONST_PATH_MAP) ? $CONST_PATH_MAP : array();

	/* 

	if (defined('WORDPRESS_INTEGRATE_PATH') && strlen(WORDPRESS_INTEGRATE_PATH)) {
		define('FINAL_WORDPRESS_INTEGRATE_PATH',WORDPRESS_INTEGRATE_PATH . ((substr(WORDPRESS_INTEGRATE_PATH, -1) == '/') ? '' : '/'));
		define('WORDPRESS_LOAD_FILE',FINAL_WORDPRESS_INTEGRATE_PATH . 'wp-load.php');

		if (!is_readable(WORDPRESS_LOAD_FILE)) {
			qa_fatal_error('Could not find wp-load.php file for WordPress integration - please checkWORDPRESS_INTEGRATE_PATH in qa-config.php');
		}
	} elseif (defined('JOOMLA_INTEGRATE_PATH') && strlen(JOOMLA_INTEGRATE_PATH)) {
		define('FINAL_JOOMLA_INTEGRATE_PATH',JOOMLA_INTEGRATE_PATH . ((substr(JOOMLA_INTEGRATE_PATH, -1) == '/') ? '' : '/'));
		define('JOOMLA_LOAD_FILE',FINAL_JOOMLA_INTEGRATE_PATH . 'configuration.php');

		if (!is_readable(JOOMLA_LOAD_FILE)) {
			qa_fatal_error('Could not find configuration.php file for Joomla integration - please checkJOOMLA_INTEGRATE_PATH in qa-config.php');
		}
	}
	...
	*/
	...
	define('PASSWORD_HASH', !qa_php_version_below('5.3.7'));
	if (PASSWORD_HASH) {
		require_once INCLUDE_DIR . 'vendor/password_compat.php';
	}
...
function qa_initialize_constants_2()
{
	...
	define('FINAL_EXTERNAL_USERS', false); 
	...
	$defaults = array(
		...
		'MAX_LIMIT_START' => 19999,
		'IGNORED_WORDS_FREQ' => 10000,
		'ALLOW_UNINDEXED_QUERIES' => false,
		'OPTIMIZE_LOCAL_DB' => true,
		'OPTIMIZE_DISTANT_DB' => false,
		'PERSISTENT_CONN_DB' => false,
...
/* 
	...
	if (defined('FINAL_WORDPRESS_INTEGRATE_PATH')) {
		define('FINAL_MYSQL_HOSTNAME', DB_HOST);
		define('FINAL_MYSQL_USERNAME', DB_USER);
		define('FINAL_MYSQL_PASSWORD', DB_PASSWORD);
		define('FINAL_MYSQL_DATABASE', DB_NAME);
		define('FINAL_EXTERNAL_USERS', true);
	...
	} elseif (defined('FINAL_JOOMLA_INTEGRATE_PATH')) {
		// More for Joomla integration
		$jconfig = new JConfig();
		define('FINAL_MYSQL_HOSTNAME', $jconfig->host);
		define('FINAL_MYSQL_USERNAME', $jconfig->user);
		define('FINAL_MYSQL_PASSWORD', $jconfig->password);
		define('FINAL_MYSQL_DATABASE', $jconfig->db);
		define('FINAL_EXTERNAL_USERS', true);
	} else {
		define('FINAL_MYSQL_HOSTNAME', MYSQL_HOSTNAME);
		define('FINAL_MYSQL_USERNAME', MYSQL_USERNAME);
		define('FINAL_MYSQL_PASSWORD', MYSQL_PASSWORD);
		define('FINAL_MYSQL_DATABASE', MYSQL_DATABASE);
		define('FINAL_EXTERNAL_USERS', EXTERNAL_USERS);
	}

	if (defined('MYSQL_PORT')) {
		define('FINAL_MYSQL_PORT', MYSQL_PORT);
	}
*/
	// Possible URL schemes for Q2A and the string used for url scheme testing

	define('URL_FORMAT_INDEX', 0);  // http://...../index.php/123/why-is-the-sky-blue
	define('URL_FORMAT_NEAT', 1);   // http://...../123/why-is-the-sky-blue [requires .htaccess]
	define('URL_FORMAT_PARAM', 3);  // http://...../?qa=123/why-is-the-sky-blue
	define('URL_FORMAT_PARAMS', 4); // http://...../?qa=123&qa_1=why-is-the-sky-blue
	define('URL_FORMAT_SAFEST', 5); // http://...../index.php?qa=123&qa_1=why-is-the-sky-blue

	define('URL_TEST_STRING', '$&-_~#%\\@^*()][`\';=:|".{},!<>?# π§½Жש'); // tests escaping, spaces, quote slashing and unicode - but not + and /
}
...
function qa_initialize_buffering($request = '')
{
	...
	$useGzip = HTML_COMPRESSION && substr($request, 0, 6) !== 'admin/' && extension_loaded('zlib');
...
{
	qa_register_module('filter', 'plugins/filter-basic.php', 'filter_basic', '');
	qa_register_module('editor', 'plugins/editor-basic.php', 'editor_basic', '');
	qa_register_module('viewer', 'plugins/viewer-basic.php', 'viewer_basic', '');
	qa_register_module('event', 'plugins/event-limits.php', 'event_limits', 'Event Limits');
	qa_register_module('event', 'plugins/event-notify.php', 'event_notify', 'Event Notify');
	qa_register_module('event', 'plugins/event-updates.php', 'event_updates', 'Event Updates');
	qa_register_module('search', 'plugins/search-basic.php', 'search_basic', '');
	qa_register_module('widget', 'plugins/widget-activity-count.php', 'activity_count', 'Activity Count');
	qa_register_module('widget', 'plugins/widget-ask-box.php', 'ask_box', 'Ask Box');
	qa_register_module('widget', 'plugins/widget-related-qs.php', 'related_qs', 'Related Questions');
	qa_register_module('widget', 'plugins/widget-category-list.php', 'category_list', 'Categories');
}
...
function qa_page_db_fail_handler($type, $errno = null, $error = null, $query = null)
{
	...
	require_once INCLUDE_DIR . 'install.php';
...
{
	$fields = array(
		'min_q2a' => 'Minimum Tabloid Version',
 ...
 */
function qa_register_module($type, $include, $class, $name, $directory = INCLUDE_DIR, $urltoroot = null)
{
	global $qa_modules;
 ...
 * @param string $urltoroot
 */
function qa_register_layer($include, $name, $directory = INCLUDE_DIR, $urltoroot = null)
{
	global $qa_layers;
 ...
 * @param string $urltoroot
 */
function qa_register_overrides($include, $directory = INCLUDE_DIR, $urltoroot = null)
{
...
function qa_register_phrases($pattern, $name)
{
	...
	if (file_exists(INCLUDE_DIR . 'langs/lang-' . $name . '.php')) {
...
function qa_eval_from_file($eval, $filename)
{
	...
	if (strlen($php_errormsg)) {
		...
		@error_log('PHP Tabloid more info: ' . $php_errormsg . " in eval()'d code from " . qa_html($filename));
	}
...
function qa_fatal_error($message)
{
	...
	echo 'Tabloid fatal error:<p style="color: red">' . qa_html($message, true) . '</p>';
	@error_log('PHP Tabloid fatal error: ' . $message);
	echo '<p>Stack trace:<p>';

	$backtrace = array_reverse(array_slice(debug_backtrace(), 1));
	foreach ($backtrace as $trace) {
		$color = strpos(@$trace['file'], '/plugins/') !== false ? 'red' : '#999';
		echo sprintf(
...
function qa_lang($identifier)
{
	...
	if (!isset($qa_phrases_full[$group])) {
		...
		if (isset($qa_lang_file_pattern[$group]))
			...
			$include = INCLUDE_DIR . 'langs/lang-' . $group . '.php';
		...
		if (strlen($languagecode)) {
			if (isset($qa_lang_file_pattern[$group]))
				...
				$include = LANGS_DIR . $languagecode . '/lang-' . $group . '.php';
		...
		$include = LANGS_DIR . 'custom/lang-' . $group . '.php';
		$phrases = is_file($include) ? (array)(include $include) : array();
		$qa_phrases_full[$group] = array_merge($qa_phrases_full[$group], $phrases);
...
/* Working with only REWRITE !

	switch ($neaturls) {

		case URL_FORMAT_INDEX:
			if (!empty($request))
				$url .= 'index.php/' . $requestpath;
			break;

		case URL_FORMAT_NEAT:
			$url .= $requestpath;
			break;

		case URL_FORMAT_PARAM:
			if (!empty($request))
...
*/
	...
	$url .= $requestpath; // URL_FORMAT_NEAT
