...
/*
	Common functions for question page viewing, either regular or via Ajax
*/
...
*/
        if ($post['basetype'] == 'Q' || $post['basetype'] == 'C' || $post['type'] == 'POST')
            $commentsfollows[$postid] = $post;
...
//	$rules['closed'] = $post['basetype'] == 'Q' && (isset($post['closedbyid']) || (isset($post['selchildid']) && qa_opt('do_close_on_select')));
	$rules['closed'] = ($post['basetype'] == 'Q' || $post['type'] == 'POST') && (isset($post['closedbyid']) || (isset($post['selchildid']) && qa_opt('do_close_on_select')));
...
//	$edit_option = $post['basetype'] == 'Q' ? 'permit_edit_q' : ($post['basetype'] == 'A' ? 'permit_edit_a' : 'permit_edit_c');
	$edit_option = ($post['basetype'] == 'Q' || $post['type'] == 'POST') ? 'permit_edit_q' : ($post['basetype'] == 'A' ? 'permit_edit_a' : 'permit_edit_c');
...
//	$rules['answerbutton'] = $post['type'] == 'Q' && $permiterror_post_a != 'level' && !$rules['closed']
    $rules['answerbutton'] = ($post['type'] == 'Q' || $post['type'] == 'POST' ) && $permiterror_post_a != 'level' && !$rules['closed']
		&& (qa_opt('allow_self_answer') || !$rules['isbyuser']);

// Zen Выключаем субкомментирование для материалов и вопросов - только для ответов и комментариев ниже!

/*
//	$rules['commentbutton'] = ($post['type'] == 'Q' || $post['type'] == 'A') && $permiterror_post_c != 'level'
	$rules['commentbutton'] = ($post['type'] == 'Q' || $post['type'] == 'A' || $post['type'] == 'POST') && $permiterror_post_c != 'level'
//		&& qa_opt($post['type'] == 'Q' ? 'comment_on_qs' : 'comment_on_as');
		&& qa_opt(($post['type'] == 'Q' || $post['type'] == 'POST') ? 'comment_on_qs' : 'comment_on_as');
	$rules['commentable'] = $rules['commentbutton'] && !$permiterror_post_c;
*/

    $rules['commentbutton'] = $post['type'] == 'A' && $permiterror_post_c != 'level' && qa_opt('comment_on_as');
    $rules['commentable'] = $rules['commentbutton'] && !$permiterror_post_c;
...
//	$rules['retagcatbutton'] = $post['basetype'] == 'Q' && (qa_using_tags() || qa_using_categories())
	$rules['retagcatbutton'] = ($post['basetype'] == 'Q' || $post['type'] == 'POST') && (qa_using_tags() || qa_using_categories())
...
//	$rules['aselectable'] = $post['type'] == 'Q' && !qa_user_permit_error($rules['isbyuser'] ? null : 'permit_select_a', null, $userlevel, true, $userfields);
	$rules['aselectable'] = ($post['type'] == 'Q' || $post['type'] == 'POST') && !qa_user_permit_error($rules['isbyuser'] ? null : 'permit_select_a', null, $userlevel, true, $userfields);
...
//		&& !($post['basetype'] == 'Q' ? $permiterror_post_q : ($post['basetype'] == 'A' ? $permiterror_post_a : $permiterror_post_c));
		&& !(($post['basetype'] == 'Q' || $post['type'] == 'POST') ? $permiterror_post_q : ($post['basetype'] == 'A' ? $permiterror_post_a : $permiterror_post_c));
...
//		if ($closepost['basetype'] == 'Q') {
		if ($closepost['basetype'] == 'Q' || $closepost['type'] == 'POST') {
			if ($closepost['hidden']) {
...
//	$questionid = ($parent['basetype'] == 'Q') ? $parent['postid'] : $parent['parentid'];
	$questionid = ($parent['basetype'] == 'Q' || $parent['type'] == 'POST') ? $parent['postid'] : $parent['parentid'];
//	$answerid = ($parent['basetype'] == 'Q') ? null : $parent['postid'];
	$answerid = ($parent['basetype'] == 'Q' || $parent['basetype'] == 'POST') ? null : $parent['postid'];
	$userid = qa_get_logged_in_userid();
	$cookieid = qa_cookie_get();
...
//				'tags' => 'name="' . (($parent['basetype'] == 'Q') ? 'q' : ('a' . qa_html($parent['postid']))) .
				'tags' => 'name="' . (($parent['basetype'] == 'Q' || $parent['basetype'] == 'POST') ? 'q' : ('a' . qa_html($parent['postid']))) .
...
//		if ($showcomment && $commentfollow['basetype'] == 'Q' && $commentfollow['hidden']) {
		if ($showcomment && ($commentfollow['basetype'] == 'Q' || $commentfollow['type'] == 'POST')  && $commentfollow['hidden']) {
...
//		} elseif ($commentfollow['basetype'] == 'Q') {
        } elseif ($commentfollow['basetype'] == 'Q' || $commentfollow['type'] == 'POST') {
...
function qa_page_q_add_a_form(&$qa_content, $formid, $captchareason, $question, $in, $errors, $loadnow, $formrequested)
{
	...
	switch (qa_user_post_permit_error('permit_post_a', $question,LIMIT_ANSWERS)) {
		case 'login':
			$form = array(
...
function qa_page_q_add_c_form(&$qa_content, $question, $parent, $formid, $captchareason, $in, $errors, $loadfocusnow)
{
	...
	switch (qa_user_post_permit_error('permit_post_c', $parent,LIMIT_COMMENTS)) {
		case 'login':
			$form = array(
