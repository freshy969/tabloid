...
/*
	More control for question page if it's submitted by HTTP POST
*/
...
if ($question['editbutton'] || $question['retagcatbutton']) {
	...
	if ($formtype == 'q_edit') { // get tags for auto-completion
		if (qa_opt('do_complete_tags'))
			$completetags = array_keys(qa_db_select_with_pending(qa_db_popular_tags_selectspec(0, DB_RETRIEVE_COMPLETE_TAGS)));
		else
			$completetags = array();
	}
}
...
function qa_page_q_edit_q_form(&$qa_content, $question, $in, $errors, $completetags, $categories)
{
    ...
    if (qa_get_logged_in_level() == USER_LEVEL_EDITOR || qa_get_logged_in_level() >= USER_LEVEL_ADMIN) {
        ...
        $value = qa_lang_html('question/the_null');
        switch ($question['type']) {
            case 'POST':
                $value = qa_lang_html('question/the_post');
                break;
            case 'Q':
                $value = qa_lang_html('question/the_question');
                break;
            case 'LINK':
                $value = qa_lang_html('question/the_link');
                break;
        }
    ...
    $fields['title'] = [
		...
		'label' => (qa_get_logged_in_level() == USER_LEVEL_EDITOR || qa_get_logged_in_level() >= USER_LEVEL_ADMIN) ? 
			qa_lang_html('question/title_label') : 
			qa_lang_html('question/q_title_label'),
        'tags' => 'name="q_title"',
    ...
    if (USE_CATEGORIES && count($categories) && $question['retagcatable']) {
    	...
    	$fields['category'] = [
            ...
            "options" => $cats,
            "value" => $cats[$question['categoryid']],
    	];
    }
...
function qa_page_q_edit_q_submit($question, $answers, $commentsfollows, $closepost, &$in, &$errors)
{
    ...
    $in['image'] = $question['image']; // Если картинка не изменилась - оставим старое значение, если изменилась - оно присвоится ниже из логики работы с $_FILES
    $in['lead'] = qa_post_text('lead');

    $in['category'] = USE_CATEGORIES ? qa_post_text('category') : null;
    $in['categoryid'] = USE_CATEGORIES ? qa_post_text('category') : null;
    ...
    if (is_array(@$_FILES['image'])) {
        ...
        elseif ($fileerror === 0 && $_FILES['image']['size'] > 0) {
            require_once INCLUDE_DIR . 'app/limits.php';
        }  
	...
	if (!qa_check_form_security_code('edit-' . $question['postid'], qa_post_text('code')))
		...
		if (empty($errors)) {
			...
			if (!$question['editable']) {
                ...
                $in['type'] = $question['type'];
                $in['image'] = $question['image'];
                $in['lead'] = $question['lead'];
			...
			qa_question_set_content($question, $in['title'], $in['content'], $in['format'], $in['text'], qa_tags_to_tagstring($in['tags']),
                ...
                $in['type'], $in['image'], $in['lead']); 

            // If there are new category for the question, update it on DB
            if (USE_CATEGORIES && strcmp($in['categoryid'], $question['categoryid'])) {
	...
	if (!qa_check_form_security_code('edit-' . $answerid, qa_post_text($prefix . 'code')))
		...
		if (empty($errors)) {
			...
			if ($in['dotoc'] && (
				...
				if (qa_user_limits_remaining(LIMIT_COMMENTS)) { // already checked 'permit_post_c'
