...
foreach ($userfields as $index => $userfield) {
	if (!($userfield['flags'] & FIELD_FLAGS_ON_REGISTER))
...
if (qa_clicked('doregister')) {
	...
	if (qa_user_limits_remaining(LIMIT_REGISTRATIONS)) {
		...
		if (!qa_check_form_security_code('register', qa_post_text('code'))) {
			...
			if (empty($errors)) {
                ...
                //  Prevent spammer registrations!
                foreach (SPAMMERS as $spammer) {
                    if ($inemail == $spammer || ($spammer[0] == '@' && strpos($inemail, $spammer) !== false)) {
                        qa_redirect('');
                    }
                }

				// register and redirect
				qa_limits_increment(null,LIMIT_REGISTRATIONS);
