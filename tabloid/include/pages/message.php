...
/*
	Controller for private messaging page
*/
...
if ($toaccount['flags'] & USER_FLAGS_NO_MESSAGES) {
...
switch (qa_user_permit_error(null,LIMIT_MESSAGES)) {
	case 'limit':
...
if (qa_post_text('domessage')) {
	...
	if (!qa_check_form_security_code('message-' . $handle, qa_post_text('code')))
		...
		if (empty($errors)) {
			require_once INCLUDE_DIR . 'db/messages.php';
			...
			$canreply = !(qa_get_logged_in_flags() & USER_FLAGS_NO_MESSAGES);
			...
			if (send_notification($toaccount['userid'], $toaccount['email'], $toaccount['handle'],
...
if (qa_opt('show_message_history')) {
	...
	$showmessages = array_slice(array_reverse($recent, true), 0, DB_RETRIEVE_MESSAGES);
