...
/*
	Controller for question page (only viewing functionality here)
*/
...
if (!isset($questionData)) {
	$questionData = qa_db_select_with_pending(
		...
		isset($userid) ? qa_db_is_favorite_selectspec($userid, ENTITY_QUESTION, $questionid) : null
	);
...
if ($question['basetype'] != 'Q' && $question['type'] != 'POST') // don't allow direct viewing of other types of post
	$question = null;

if (isset($question)) {

    // Inlining view increment from qa-include/db/hotness.php
    if (COUNT_VIEWS) {
        //qa_db_increment_views($question['postid']);
        ...
        qa_db_query_sub($query, $ipHex, $question['postid'], $ipHex);
    }
...
if (qa_is_http_post() || strlen($pagestate))
	require INCLUDE_DIR . 'pages/question-post.php';
...
$keys = []; 
if (isset($question) && is_array($question) && array_key_exists('categoryid', $question)) 
    $keys = array_keys(qa_category_path($categories, $question['categoryid']));
$qa_content = qa_content_prepare(true, $keys);

if (isset($userid) && !$formrequested)
	$qa_content['favorite'] = qa_favorite_form(ENTITY_QUESTION, $questionid, $favorite,
...
elseif ($question['queued'])
	$qa_content['error'] = $question['isbyuser'] ? 
		qa_lang_html('question/q_your_waiting_approval') : 
		qa_lang_html('question/q_waiting_your_approval');
...
if (qa_opt('sort_answers_by') == 'votes') {
	...
	usort($answers, function ($item1, $item2) {
    	return intval($item1['netvotes']) <= intval($item2['netvotes']);
	});
...
if ($question['basetype'] == 'Q' || $question['type'] == 'POST') {
