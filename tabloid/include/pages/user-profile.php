...
/*
	Controller for user profile page, including wall
*/
...
list($useraccount, $userprofile, $userfields, $usermessages, $userpoints, $userlevels, $navcategories, $userrank) =
	qa_db_select_with_pending(
		...
		FINAL_EXTERNAL_USERS ? null : qa_db_user_profile_selectspec($handle, false),
		FINAL_EXTERNAL_USERS ? null : qa_db_userfields_selectspec(),
		FINAL_EXTERNAL_USERS ? null : qa_db_recent_messages_selectspec(null, null, $handle, false, qa_opt_if_loaded('page_size_wall')),
		qa_db_user_points_selectspec($identifier),
		qa_db_user_levels_selectspec($identifier, FINAL_EXTERNAL_USERS, true),
		qa_db_category_nav_selectspec(null, true),
		qa_db_user_rank_selectspec($identifier)
	);

if (!FINAL_EXTERNAL_USERS && $handle !== qa_get_logged_in_handle()) {
...
if (!FINAL_EXTERNAL_USERS) { // if we're using integrated user management, we can know and show more
	...
	if (isset($loginuserid) && $loginuserid != $userid &&
		($loginlevel >= USER_LEVEL_SUPER || $loginlevel > $maxuserlevel) &&
		!qa_user_permit_error()
	) { // can't change self - or someone on your level (or higher, obviously) unless you're a super admin
		if ($loginlevel >= USER_LEVEL_SUPER)
			$maxlevelassign = USER_LEVEL_SUPER;
		elseif ($loginlevel >= USER_LEVEL_ADMIN)
			$maxlevelassign = USER_LEVEL_MODERATOR;
		elseif ($loginlevel >= USER_LEVEL_MODERATOR)
			$maxlevelassign = USER_LEVEL_EXPERT;

		if ($loginlevel >= USER_LEVEL_ADMIN)
			$fieldseditable = true;

		if (isset($maxlevelassign) && ($useraccount['flags'] & USER_FLAGS_USER_BLOCKED))
			$maxlevelassign = min($maxlevelassign, USER_LEVEL_EDITOR); // if blocked, can't promote too high
	}

	$approvebutton = isset($maxlevelassign)
		&& $useraccount['level'] < USER_LEVEL_APPROVED
		&& $maxlevelassign >= USER_LEVEL_APPROVED
		&& !($useraccount['flags'] & USER_FLAGS_USER_BLOCKED)
		...
		if (qa_clicked('docancel')) {
			...
			if (!qa_check_form_security_code('user-edit-' . $handle, qa_post_text('code'))) {
				...
				if (qa_post_text('removeavatar')) {
					qa_db_user_set_flag($userid, USER_FLAGS_SHOW_AVATAR, false);
					qa_db_user_set_flag($userid, USER_FLAGS_SHOW_GRAVATAR, false);
				...
				if ($fieldseditable) {
					...
					if (!isset($errors['email'])) {
						if ($inemail != $useraccount['email']) {
							...
							qa_db_user_set_flag($userid, USER_FLAGS_EMAIL_CONFIRMED, false);
						}
					}
	...
	if (qa_clicked('doapprove') || qa_clicked('doblock') || qa_clicked('dounblock') || qa_clicked('dohideall') || qa_clicked('dodelete')) {
		if (!qa_check_form_security_code('user-' . $handle, qa_post_text('code')))
			...
			if ($approvebutton && qa_clicked('doapprove')) {
				require_once INCLUDE_DIR . 'app/users-edit.php';
				qa_set_user_level($userid, $useraccount['handle'], USER_LEVEL_APPROVED, $useraccount['level']);
				qa_redirect(qa_request());
			}

			if (isset($maxlevelassign) && ($maxuserlevel < USER_LEVEL_MODERATOR)) {
				...
				if (qa_clicked('dohideall') && !qa_user_permit_error('permit_hide_show')) {
					require_once INCLUDE_DIR . 'db/admin.php';
					require_once INCLUDE_DIR . 'app/posts.php';

					$postids = qa_db_get_user_visible_postids($userid);

					foreach ($postids as $postid)
						qa_post_set_status($postid, POST_STATUS_HIDDEN, $loginuserid);

					qa_redirect(qa_request());
				}

				if (qa_clicked('dodelete') && ($loginlevel >= USER_LEVEL_ADMIN)) {
					require_once INCLUDE_DIR . 'app/users-edit.php';
...
if ($loginlevel >= USER_LEVEL_ADMIN && qa_clicked('dosetbonus')) {
...
if (isset($loginuserid) && $loginuserid != $useraccount['userid'] && !FINAL_EXTERNAL_USERS) {
	$favoritemap = qa_get_favorite_non_qs_map();
	$favorite = @$favoritemap['user'][$useraccount['userid']];

	$qa_content['favorite'] = qa_favorite_form(ENTITY_USER, $useraccount['userid'], $favorite,
...
if (!FINAL_EXTERNAL_USERS) {
	...
	$qa_content['form_profile'] = array(
		...
		'fields' => array(
			...
			'level' => array(
				...
				'note' => (($useraccount['flags'] & USER_FLAGS_USER_BLOCKED) && isset($maxlevelassign)) ? qa_lang_html('users/user_blocked') : '',
				'id' => 'level',
			),
		),
	);
	...
	if (qa_opt('allow_private_messages') && isset($loginuserid) && $loginuserid != $userid && !($useraccount['flags'] & USER_FLAGS_NO_MESSAGES) && !$userediting) {
	...
	if ($userediting) {
		if (isset($maxlevelassign)) {
			...
			$showlevels = array(USER_LEVEL_BASIC);
			if (qa_opt('moderate_users'))
				$showlevels[] = USER_LEVEL_APPROVED;

			array_push($showlevels, USER_LEVEL_EXPERT, USER_LEVEL_EDITOR, USER_LEVEL_MODERATOR, USER_LEVEL_ADMIN, USER_LEVEL_SUPER);
			...
			if (qa_using_categories()) {
				...
				if ($catleveladd || count($userlevels))
					$userlevels[] = array('entitytype' => ENTITY_CATEGORY);

				$index = 0;
				foreach ($userlevels as $userlevel) {
					if ($userlevel['entitytype'] == ENTITY_CATEGORY) {
						$index++;
						$id = 'ls_' . +$index;
		...
		foreach ($userlevels as $userlevel) {
			if ($userlevel['entitytype'] == ENTITY_CATEGORY && $userlevel['level'] > $useraccount['level']) {
	...
	foreach ($permitoptions as $permitoption) {
		...
		if (qa_permit_error($permitoption, $userid, USER_LEVEL_APPROVED, USER_FLAGS_EMAIL_CONFIRMED, 0) &&
	...
	if ($loginlevel >= USER_LEVEL_ADMIN && !qa_user_permit_error()) {
		$doconfirms = qa_opt('confirm_user_emails') && $useraccount['level'] < USER_LEVEL_EXPERT;
		$isconfirmed = ($useraccount['flags'] & USER_FLAGS_EMAIL_CONFIRMED) > 0;
	...
	if ($loginlevel >= USER_LEVEL_MODERATOR && !qa_user_permit_error()) {
	...
	foreach ($userfields as $userfield) {
		if (($userfield['flags'] & FIELD_FLAGS_LINK_URL) && !$fieldsediting) {
			...
			$valuehtml = qa_html($value, (($userfield['flags'] & FIELD_FLAGS_MULTI_LINE) && !$fieldsediting));
		}
		...
		if (isset($userfield['permit']) && !$userediting) {
			if ($userfield['permit'] <=PERMIT_ADMINS)
				$notehtml = qa_lang_html('users/only_shown_admins');
			elseif ($userfield['permit'] <=PERMIT_MODERATORS)
				$notehtml = qa_lang_html('users/only_shown_moderators');
			elseif ($userfield['permit'] <=PERMIT_EDITORS)
				$notehtml = qa_lang_html('users/only_shown_editors');
			elseif ($userfield['permit'] <=PERMIT_EXPERTS)
		...
		$qa_content['form_profile']['fields'][$userfield['title']] = array(
			...
			'note' => $notehtml,
			'rows' => ($userfield['flags'] & FIELD_FLAGS_MULTI_LINE) ? 8 : null,
	...
	if ($userediting) {
		if ((qa_opt('avatar_allow_gravatar') && ($useraccount['flags'] & USER_FLAGS_SHOW_GRAVATAR)) ||
			(qa_opt('avatar_allow_upload') && ($useraccount['flags'] & USER_FLAGS_SHOW_AVATAR) && isset($useraccount['avatarblobid']))
		) {
		...
		if (isset($maxlevelassign) && $useraccount['level'] < USER_LEVEL_MODERATOR) {
			if ($useraccount['flags'] & USER_FLAGS_USER_BLOCKED) {
...
if (!FINAL_EXTERNAL_USERS && qa_opt('allow_user_walls')) {
