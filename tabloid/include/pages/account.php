...
/*
	Controller for user account page
*/
...
$isconfirmed = ($useraccount['flags'] & USER_FLAGS_EMAIL_CONFIRMED) ? true : false;
	...
	if (qa_clicked('dosaveprofile') && !$isblocked) {
		...
		if (!qa_check_form_security_code('account', qa_post_text('code')))
			...
			if (qa_opt('allow_private_messages'))
				qa_db_user_set_flag($userid, USER_FLAGS_NO_MESSAGES, !$inmessages);

			if (qa_opt('allow_user_walls'))
				qa_db_user_set_flag($userid, USER_FLAGS_NO_WALL_POSTS, !$inwallposts);

			if (qa_opt('mailing_enabled'))
				qa_db_user_set_flag($userid, USER_FLAGS_NO_MAILINGS, !$inmailings);

			qa_db_user_set_flag($userid, USER_FLAGS_SHOW_AVATAR, ($inavatar == 'uploaded'));
			qa_db_user_set_flag($userid, USER_FLAGS_SHOW_GRAVATAR, ($inavatar == 'gravatar'));
...
$qa_content['form_profile'] = array(
	...
	'fields' => array(
		...
		'messages' => array(
			...
			'type' => 'checkbox',
			'value' => !($useraccount['flags'] & USER_FLAGS_NO_MESSAGES),
		...
		'wall' => array(
			...
			'type' => 'checkbox',
			'value' => !($useraccount['flags'] & USER_FLAGS_NO_WALL_POSTS),
		...
		'mailings' => array(
			...
			'tags' => 'name="mailings"',
			'type' => 'checkbox',
			'value' => !($useraccount['flags'] & USER_FLAGS_NO_MAILINGS),
...
if (qa_opt('avatar_allow_gravatar') || qa_opt('avatar_allow_upload')) {
	...
	if (qa_opt('avatar_allow_gravatar') && !$pending_confirmation) {
		...
		if ($useraccount['flags'] & USER_FLAGS_SHOW_GRAVATAR)
	...
	if (qa_has_gd_image() && qa_opt('avatar_allow_upload') && !$pending_confirmation) {
		...
		if ($useraccount['flags'] & USER_FLAGS_SHOW_AVATAR)
