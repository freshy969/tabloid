...
if ((qa_clicked('doaddpage') || qa_clicked('doaddlink') || qa_get('doaddlink') || qa_clicked('dosavepage')) && !isset($editpage)) {
	...
	$isexternal = $editpage['flags'] & PAGE_FLAGS_EXTERNAL;
...
elseif (qa_clicked('dosaveoptions') || qa_clicked('doaddpage') || qa_clicked('doaddlink')) {
	...
	if (!qa_check_form_security_code('admin/pages', qa_post_text('code')))
		...
		if (qa_post_text('dodelete')) {
			...
			elseif (qa_strlen($inname) > DB_MAX_CAT_PAGE_TITLE_LENGTH)
				$errors['name'] = qa_lang_sub('main/max_length_x', DB_MAX_CAT_PAGE_TITLE_LENGTH);
			...
			if ($isexternal) {
				...
				elseif (qa_strlen($inurl) > DB_MAX_CAT_PAGE_TAGS_LENGTH)
					$errors['url'] = qa_lang_sub('main/max_length_x', DB_MAX_CAT_PAGE_TAGS_LENGTH);

			} else {
				// Verify the heading is legitimate

				if (qa_strlen($inheading) > DB_MAX_TITLE_LENGTH)
					$errors['heading'] = qa_lang_sub('main/max_length_x', DB_MAX_TITLE_LENGTH);
				...
				for ($attempt = 0; $attempt < 100; $attempt++) {
					...
					elseif (qa_strlen($inslug) > DB_MAX_CAT_PAGE_TAGS_LENGTH)
						$errors['slug'] = qa_lang_sub('main/max_length_x', DB_MAX_CAT_PAGE_TAGS_LENGTH);
			...
			if (isset($editpage['pageid'])) { // changing existing page
				if ($isexternal) {
					qa_db_page_set_fields($editpage['pageid'],
						...
						PAGE_FLAGS_EXTERNAL | ($innewwindow ? PAGE_FLAGS_NEW_WINDOW : 0),
				...
				if (empty($errors)) {
					if ($isexternal) {
						$pageid = qa_db_page_create($inname, PAGE_FLAGS_EXTERNAL | ($innewwindow ? PAGE_FLAGS_NEW_WINDOW : 0), $inurl, null, null, $inpermit);
					} else {
...
if (isset($editpage)) {
	...
	$permitoptions = qa_admin_permit_options(PERMIT_ALL,PERMIT_ADMINS, false, false);
	...
	$qa_content['form'] = array(
		...
		'fields' => array(
			...
			'newwindow' => array(
				...
				'value' => (isset($innewwindow) ? $innewwindow : (@$editpage['flags'] & PAGE_FLAGS_NEW_WINDOW)) ? 1 : 0,
				'type' => 'checkbox',
			),
	...
	foreach ($pages as $page) {
		...
		$listhtml .= strtr(qa_lang_html(($page['flags'] & PAGE_FLAGS_EXTERNAL) ? 'admin/edit_link' : 'admin/edit_page'), array(
