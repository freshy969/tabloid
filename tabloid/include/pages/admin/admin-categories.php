...
if ($setmissing) {
	...
	if ($setparent) {
		...
		qa_set_up_category_field($qa_content, $qa_content['form']['fields']['parent'], 'parent',
			...
			true, true, CATEGORY_DEPTH - 1 - $childdepth, @$editcategory['categoryid']);

		$qa_content['form']['fields']['parent']['options'][''] = qa_lang_html('admin/category_top_level');

		@$qa_content['form']['fields']['parent']['note'] .= qa_lang_html_sub('admin/category_max_depth_x', CATEGORY_DEPTH);
	...
	if (!$setparent) {
		...
		if (isset($editcategory['categoryid'])) {
			...
			if ($catdepth < CATEGORY_DEPTH) {
				...
				$qa_content['form']['fields']['name']['note'] = qa_lang_html_sub('admin/category_no_add_subs_x', CATEGORY_DEPTH);
			}
