...
elseif (qa_clicked('dosavefield')) {
	...
	if (!qa_check_form_security_code('admin/userfields', qa_post_text('code')))
		...
		if (qa_post_text('dodelete')) {
			...
			$inflags = $intype | ($inonregister ? FIELD_FLAGS_ON_REGISTER : 0);
			...
			if (qa_strlen($inname) > DB_MAX_PROFILE_TITLE_LENGTH)
				$errors['name'] = qa_lang_sub('main/max_length_x', DB_MAX_PROFILE_TITLE_LENGTH);
			...
			if (isset($editfield['fieldid'])) { // changing existing user field
				...
				for ($attempt = 0; $attempt < 1000; $attempt++) {
					...
					$newtag = qa_substr(implode('-', qa_string_to_words($inname)), 0, DB_MAX_PROFILE_TITLE_LENGTH - strlen($suffix)) . $suffix;
					$uniquetag = true;
...
$typeoptions = array(
	...
	FIELD_FLAGS_MULTI_LINE => qa_lang_html('admin/field_multi_line'),
	FIELD_FLAGS_LINK_URL => qa_lang_html('admin/field_link_url'),
);
...
$qa_content['form'] = array(
	...
	'fields' => array(
		...
		'type' => array(
			...
			'type' => 'select',
			'options' => $typeoptions,
			'value' => @$typeoptions[isset($intype) ? $intype : (@$editfield['flags'] & (FIELD_FLAGS_MULTI_LINE | FIELD_FLAGS_LINK_URL))],
		),
		...
		'onregister' => array(
			...
			'type' => 'checkbox',
			'value' => isset($inonregister) ? $inonregister : (@$editfield['flags'] & FIELD_FLAGS_ON_REGISTER),
		),
	),
