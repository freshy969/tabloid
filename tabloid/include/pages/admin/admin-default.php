...
/*
	Controller for most admin pages which just contain options
*/
...
$optionmaximum = array(
	'feed_number_items' => DB_RETRIEVE_QS_AS,
	'max_len_q_title' => DB_MAX_TITLE_LENGTH,
	'page_size_activity' => DB_RETRIEVE_QS_AS,
	'page_size_ask_check_qs' => DB_RETRIEVE_QS_AS,
	'page_size_ask_tags' => DB_RETRIEVE_QS_AS,
	'page_size_home' => DB_RETRIEVE_QS_AS,
	'page_size_hot_qs' => DB_RETRIEVE_QS_AS,
	'page_size_pms' => DB_RETRIEVE_MESSAGES,
	'page_size_qs' => DB_RETRIEVE_QS_AS,
	'page_size_related_qs' => DB_RETRIEVE_QS_AS,
	'page_size_search' => DB_RETRIEVE_QS_AS,
	'page_size_tag_qs' => DB_RETRIEVE_QS_AS,
	'page_size_tags' => DB_RETRIEVE_TAGS,
	'page_size_una_qs' => DB_RETRIEVE_QS_AS,
	'page_size_users' => DB_RETRIEVE_USERS,
	'page_size_wall' => DB_RETRIEVE_MESSAGES,
);
...
*/		
	...
	case 'permissions':
		...
		foreach ($permitoptions as $permitoption) {
			...
			if ($permitoption == 'permit_view_q_page') {
				...
				$checkboxtodisplay['allow_view_q_bots'] = 'option_permit_view_q_page<' . qa_js(PERMIT_ALL);

			} else {
				$showoptions[] = $permitoption . '_points';
				$checkboxtodisplay[$permitoption . '_points'] = '(option_' . $permitoption . '==' . qa_js(PERMIT_POINTS) .
					')||(option_' . $permitoption . '==' . qa_js(PERMIT_POINTS_CONFIRMED) . ')||(option_' . $permitoption . '==' . qa_js(PERMIT_APPROVED_POINTS) . ')';
			}
		}
	...
	case 'spam':
		...
		if (count($captchamodules)) {
			...
			if ($maxpermitpost >PERMIT_USERS)
				$showoptions[] = 'captcha_on_anon_post';

			if ($maxpermitpost >PERMIT_APPROVED)
				$showoptions[] = 'captcha_on_unapproved';

			if ($maxpermitpost >PERMIT_CONFIRMED)
		...
		if ($maxpermitpost >PERMIT_EXPERTS)
...
$qa_content['script_rel'][] = '/js/admin.js';
...
/* 
					$neatoptions[$rawoption] =
						'<iframe src="' . qa_path_html('url/test/' . URL_TEST_STRING, array('dummy' => '', 'param' => URL_TEST_STRING), null, $rawoption) . '" width="20" height="16" style="vertical-align:middle; border:0" scrolling="no"></iframe>&nbsp;' .
						'<small>' .
						qa_html(urldecode(qa_path('123/why-do-birds-sing', null, '/', $rawoption))) .
						(($rawoption == URL_FORMAT_NEAT) ? strtr(qa_lang_html('admin/neat_urls_note'), array(
							'^1' => '<a href="http://www.tabloid.dev/htaccess.php" target="_blank">',
							'^2' => '</a>',
						)) : '') .
						'</small>';
*/
				...
				qa_optionfield_make_select($optionfield, $neatoptions, $value, URL_FORMAT_SAFEST);
			...
			case 'site_theme_mobile':
				...
				if (!isset($themeoptions[$value]))
					$value = 'zen'; // check here because we also need $value for qa_addon_metadata()

				qa_optionfield_make_select($optionfield, $themeoptions, $value, 'Classic');

				$metadataUtil = new Q2A_Util_Metadata();
				$themedirectory = THEMES_DIR . $value;
...
//					$contents = @file_get_contents($themedirectory . '/qa-styles.css', false, null, 0, 8192);
					$contents = @file_get_contents($themedirectory . '/styles.css', false, null, 0, 8192);
