...
/*
	Common functions for connecting to and accessing database
*/
...
/* 
	...
	} elseif (defined('FINAL_MYSQL_PORT')) {
		$port = FINAL_MYSQL_PORT;
	}

	if (PERSISTENT_CONN_DB)
		$host = 'p:' . $host;
*/
/* 
	// in mysqli we connect and select database in constructor
	if ($port !== null)
		$db = new mysqli($host, FINAL_MYSQL_USERNAME, FINAL_MYSQL_PASSWORD, FINAL_MYSQL_DATABASE, $port);
	else
		$db = new mysqli($host, FINAL_MYSQL_USERNAME, FINAL_MYSQL_PASSWORD, FINAL_MYSQL_DATABASE);
*/
...
function qa_db_fail_error($type, $errno = null, $error = null, $query = null)
{
	...
	@error_log('PHP Tabloid MySQL ' . $type . ' error ' . $errno . ': ' . $error . (isset($query) ? (' - Query: ' . $query) : ''));
...
function qa_db_query_raw($query)
{
	...
	// @error_log('Tabloid MySQL query: '.$query);
 ...
 *MYSQL_TABLE_PREFIX or if it should include all tables in the database.
 * @return array
 */
function qa_db_list_tables($onlyTablesWithPrefix = false)
{
	...
	if ($onlyTablesWithPrefix) {
		...
		$query .= ' WHERE `' . $col . '` LIKE "' . str_replace('_', '\\_', TABLE_PREFIX) . '%"';
		if (defined('USERS_PREFIX')) {
			$query .= ' OR `' . $col . '` LIKE "' . str_replace('_', '\\_', MYSQL_USERS_PREFIX) . '%"';
		}
	}
...
/*
	...
	IfOPTIMIZE_DISTANT_DB is set to false in qa-config.php, we assume zero latency and go back to
...
function qa_db_multi_select($selectspecs)
{
	...
	if (!OPTIMIZE_DISTANT_DB || (count($selectspecs) <= 1)) {
		$outresults = array();
