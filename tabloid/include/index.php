...
/*
	The Grand Central of Tabloid - most requests come through here
*/
...
elseif (isset($_GET['qa']) && $_GET['qa'] == 'blob') {
	 ...
	 */
		...
		if (isset($_GET['rewrite'])) { // URLs rewritten by .htaccess or Nginx [WAS qa-rewrite]

			$urlformat = URL_FORMAT_NEAT;
			$rewrite = strtr(qa_gpc_to_string($_GET['rewrite']), '+', ' '); // strtr required by Nginx
			$requestparts = explode('/', $rewrite);
			unset($_GET['rewrite']);
			...
			if (!empty($_SERVER['REQUEST_URI'])) { // workaround for the fact that Apache unescapes characters while rewriting
				...
				// Generally we assume that $_GET['rewrite'] has the right path depth, but this won't be the case if there's
				...
				if (substr($normalizedpath, -strlen($rewrite)) !== $rewrite) {
			...
			if (strpos($_GET['qa'], '/') === false) {
				$urlformat = (empty($_SERVER['REQUEST_URI']) || strpos($_SERVER['REQUEST_URI'], '/index.php') !== false)
					? URL_FORMAT_SAFEST : URL_FORMAT_PARAMS;
...
//	} elseif ($requestlower == 'url/test/' . URL_TEST_STRING) {
//		require INCLUDE_DIR . 'url-test.php';
//	} else {
