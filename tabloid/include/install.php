...
/*
	User interface for installing, upgrading and fixing the database
*/

require_once INCLUDE_DIR.'db/install.php';
...
if (!function_exists('qa_install_db_fail_handler')) {
	...
	function qa_install_db_fail_handler($type, $errno = null, $error = null, $query = null)
	{
		...
		require INCLUDE_DIR . 'install.php';
...
if (!isset($pass_failure_type) && qa_clicked('super')) {
	...
	if (qa_db_count_users() == 0) { // prevent creating multiple accounts
		...
		if (empty($fielderrors)) {
			...
			$userid = qa_create_new_user($inemail, $inpassword, $inhandle, USER_LEVEL_SUPER);
			qa_set_logged_in_user($userid, $inhandle);

			qa_set_option('feedback_email', $inemail);

			$success .= "Congratulations - Your Tabloid site is ready to go!\n\nYou are logged in as the super administrator and can start changing settings.\n\nThank you for installing Tabloid.";
		}
	}
}
...
if (isset($pass_failure_type)) {
	...
	switch ($pass_failure_type) {
		...
		case 'select':
			$errorhtml .= 'Could not switch to the Tabloid database. Please check the database name in the config file, and if necessary create the database in MySQL and grant appropriate user privileges.';
			break;
		...
		case 'query':
			...
			if (@$pass_failure_from_install)
				$errorhtml .= "Tabloid was unable to perform the installation query below. Please check the user in the config file has CREATE and ALTER permissions:\n\n".qa_html($pass_failure_query."\n\nError ".$pass_failure_errno.": ".$pass_failure_error."\n\n");
			else
				$errorhtml .= "A Tabloid database query failed when generating this page.\n\nA full description of the failure is available in the web server's error log file.";
			break;
	}
}
else {
	...
	if (qa_clicked('create')) {
		...
		if (FINAL_EXTERNAL_USERS) {
			if (defined('FINAL_WORDPRESS_INTEGRATE_PATH')) {
				...
				qa_db_page_move(qa_db_page_create(get_option('blogname'), PAGE_FLAGS_EXTERNAL, get_option('home'), null, null, null), 'O', 1);

				$success .= 'Your Tabloid database has been created and integrated with your WordPress site.';
			...
			elseif (defined('FINAL_JOOMLA_INTEGRATE_PATH')) {
				...
				qa_db_page_move(qa_db_page_create($jconfig->sitename, PAGE_FLAGS_EXTERNAL, '../', null, null, null), 'O', 1);
				$success .= 'Your Tabloid database has been created and integrated with your Joomla! site.';
			}
			else {
				$success .= 'Your Tabloid database has been created for external user identity management. Please read the online documentation to complete integration.';
			}
		}
		else {
			$success .= 'Your Tabloid database has been created.';
		}
	}

	if (qa_clicked('nonuser')) {
		qa_db_install_tables();
		$success .= 'The additional Tabloid database tables have been created.';
	}

	if (qa_clicked('upgrade')) {
		qa_db_upgrade_tables();
		$success .= 'Your Tabloid database has been updated.';
	}

	if (qa_clicked('repair')) {
		qa_db_install_tables();
		$success .= 'The Tabloid database tables have been repaired.';
	}
...
if (qa_db_connection(false) !== null && !@$pass_failure_from_install) {
	...
	switch ($check) {
		case 'none':
			...
			$errorhtml .= 'Welcome to Tabloid. It\'s time to set up your database!';

			if (FINAL_EXTERNAL_USERS) {
				if (defined('FINAL_WORDPRESS_INTEGRATE_PATH')) {
					$errorhtml .= "\n\nWhen you click below, your Tabloid site will be set up to integrate with the users of your WordPress site <a href=\"".qa_html(get_option('home'))."\" target=\"_blank\">".qa_html(get_option('blogname'))."</a>. Please consult the online documentation for more information.";
				}
				elseif (defined('FINAL_JOOMLA_INTEGRATE_PATH')) {
					$jconfig = new JConfig();
					$errorhtml .= "\n\nWhen you click below, your Tabloid site will be set up to integrate with the users of your Joomla! site <a href=\"../\" target=\"_blank\">".$jconfig->sitename."</a>. It's also recommended to install the Joomla QAIntegration plugin for additional user-access control. Please consult the online documentation for more information.";
				}
				else {
					$errorhtml .= "\n\nWhen you click below, your Tabloid site will be set up to integrate with your existing user database and management. Users will be referenced with database column type ".qa_html(qa_get_mysql_user_column_type()).". Please consult the online documentation for more information.";
				}

				$buttons = array('create' => 'Set up the Database');
			}
			else {
				$errorhtml .= "\n\nWhen you click below, your Tabloid database will be set up to manage user identities and logins internally.\n\nIf you want to offer a single sign-on for an existing user base or website, please consult the online documentation before proceeding.";
		...
		case 'old-version':
			...
			$errorhtml .= 'Your Tabloid database needs to be upgraded for this version of the software.';
			$buttons = array('upgrade' => 'Upgrade the Database');
			break;

		case 'non-users-missing':
			$errorhtml = 'This Tabloid site is sharing its users with another Q2A site, but it needs some additional database tables for its own content. Please click below to create them.';
			$buttons = array('nonuser' => 'Set up the Tables');
			break;

		case 'table-missing':
			$errorhtml .= 'One or more tables are missing from your Tabloid database.';
			$buttons = array('repair' => 'Repair the Database');
			break;

		case 'column-missing':
			$errorhtml .= 'One or more Tabloid database tables are missing a column.';
			$buttons = array('repair' => 'Repair the Database');
			break;
		...
		default:
			...
			if (!FINAL_EXTERNAL_USERS && qa_db_count_users() == 0) {
				$errorhtml .= "There are currently no users in the Tabloid database.\n\nPlease enter your details below to create the super administrator:";
				$fields = array(
...
if (empty($errorhtml)) {
	if (empty($success))
		$success = 'Your Tabloid database has been checked with no problems.';

	$suggest = '<a href="'.qa_path_html('admin', null, null, URL_FORMAT_SAFEST).'">Go to admin center</a>';
}

?>

		<form method="post" action="<?php echo qa_path_html('install', null, null, URL_FORMAT_SAFEST)?>">
