...
/*
	Handling incoming votes (application level)
*/
...
function qa_vote_error_html($post, $vote, $userid, $topage)
{
	...
	$permiterror = qa_user_post_permit_error(($post['basetype'] == 'Q') ? 'permit_vote_q' : 'permit_vote_a', $post,LIMIT_VOTES);
...
function qa_vote_set($post, $userid, $handle, $cookieid, $vote)
{
	...
	if (!in_array($post['basetype'], ['Q', 'A', 'C', 'P'])) 
		return;
...
function qa_flag_error_html($post, $userid, $topage)
{
	...
	if (is_array($post) && qa_opt('flagging_of_posts') &&
		...
		switch (qa_user_post_permit_error('permit_flag', $post,LIMIT_FLAGS)) {
			case 'login':
