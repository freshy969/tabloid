...
/*
	Functions used in the admin center pages
*/
...
function qa_admin_check_privileges(&$qa_content)
{
	...
	} elseif (qa_get_logged_in_level() < USER_LEVEL_ADMIN) {
		$qa_content = qa_content_prepare();
...
function qa_admin_language_options()
{
	...
	$options = array('' => 'English');

	// find all language folders
	$metadataUtil = new Q2A_Util_Metadata();
	foreach (glob(LANGS_DIR . '*', GLOB_ONLYDIR) as $directory) {
...
function qa_admin_theme_options()
{
	...
	$metadataUtil = new Q2A_Util_Metadata();
	foreach (glob(THEMES_DIR . '*', GLOB_ONLYDIR) as $directory) {
		...
		if (empty($metadata)) {
			...
			$contents = @file_get_contents($directory . '/styles.css', false, null, 0, 8192);
...
function qa_admin_permit_options($widest, $narrowest, $doconfirms = true, $dopoints = true)
{
	...
	$options = array(
		PERMIT_ALL => qa_lang_html('options/permit_all'),
		PERMIT_USERS => qa_lang_html('options/permit_users'),
		PERMIT_CONFIRMED => qa_lang_html('options/permit_confirmed'),
		PERMIT_POINTS => qa_lang_html('options/permit_points'),
		PERMIT_POINTS_CONFIRMED => qa_lang_html('options/permit_points_confirmed'),
		PERMIT_APPROVED => qa_lang_html('options/permit_approved'),
		PERMIT_APPROVED_POINTS => qa_lang_html('options/permit_approved_points'),
		PERMIT_EXPERTS => qa_lang_html('options/permit_experts'),
		PERMIT_EDITORS => qa_lang_html('options/permit_editors'),
		PERMIT_MODERATORS => qa_lang_html('options/permit_moderators'),
		PERMIT_ADMINS => qa_lang_html('options/permit_admins'),
		PERMIT_SUPERS => qa_lang_html('options/permit_supers'),
	);
	...
	if (!$doconfirms) {
		unset($options[PERMIT_CONFIRMED]);
		unset($options[PERMIT_POINTS_CONFIRMED]);
	}

	if (!$dopoints) {
		unset($options[PERMIT_POINTS]);
		unset($options[PERMIT_POINTS_CONFIRMED]);
		unset($options[PERMIT_APPROVED_POINTS]);
	}

	if (FINAL_EXTERNAL_USERS || !qa_opt('moderate_users')) {
		unset($options[PERMIT_APPROVED]);
		unset($options[PERMIT_APPROVED_POINTS]);
	}
...
function qa_admin_sub_navigation()
{
	...
	if (!FINAL_EXTERNAL_USERS && qa_opt('moderate_users') && $level >= USER_LEVEL_MODERATOR) {
...
{
	if (file_exists(INCLUDE_DIR . 'db/install.php')) // file can be removed for extra security
		include_once INCLUDE_DIR . 'db/install.php';

	if (defined('DB_VERSION_CURRENT') && qa_opt('db_version') < DB_VERSION_CURRENT && qa_get_logged_in_level() >= USER_LEVEL_ADMIN) {
		return strtr(
	...
	} elseif (defined('BLOBS_DIRECTORY') && !is_writable(BLOBS_DIRECTORY)) {
		return qa_lang_html_sub('admin/blobs_directory_error', qa_html(BLOBS_DIRECTORY));
	}
...
function qa_admin_single_click($entityid, $action)
{
	...
	if (!FINAL_EXTERNAL_USERS && ($action == 'userapprove' || $action == 'userblock')) { // approve/block moderated users
		require_once INCLUDE_DIR . 'db/selects.php';

		$useraccount = qa_db_select_with_pending(qa_db_user_account_selectspec($entityid, true));

		if (isset($useraccount) && qa_get_logged_in_level() >= USER_LEVEL_MODERATOR) {
			switch ($action) {
				case 'userapprove':
					if ($useraccount['level'] <= USER_LEVEL_APPROVED) { // don't demote higher level users
						require_once INCLUDE_DIR . 'app/users-edit.php';
						qa_set_user_level($useraccount['userid'], $useraccount['handle'], USER_LEVEL_APPROVED, $useraccount['level']);
						return true;
					}
					break;
			...
			switch ($action) {
				case 'approve':
					if ($queued && !qa_user_post_permit_error('permit_moderate', $post)) {
						qa_post_set_status($entityid, POST_STATUS_NORMAL, $userid);
						return true;
					}
					break;
				...
				case 'reject':
					if ($queued && !qa_user_post_permit_error('permit_moderate', $post)) {
						qa_post_set_status($entityid, POST_STATUS_HIDDEN, $userid);
						return true;
					}
					break;
