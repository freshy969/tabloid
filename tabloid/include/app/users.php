...
/*
	User management (application level) for basic user operations
*/

define('USER_LEVEL_BASIC',     0);   
define('USER_LEVEL_APPROVED',  10);  
define('USER_LEVEL_EXPERT',    20);
define('USER_LEVEL_COMPANY',   40);  
define('USER_LEVEL_MODERATOR', 60);  
define('USER_LEVEL_EDITOR',    80);
define('USER_LEVEL_ADMIN',     100);
define('USER_LEVEL_SUPER',     120);

define('USER_FLAGS_EMAIL_CONFIRMED', 1);
define('USER_FLAGS_USER_BLOCKED',    2);
define('USER_FLAGS_SHOW_AVATAR',     4);
define('USER_FLAGS_SHOW_GRAVATAR',   8);
define('USER_FLAGS_NO_MESSAGES',     16);
define('USER_FLAGS_NO_MAILINGS',     32);
define('USER_FLAGS_WELCOME_NOTICE',  64);
define('USER_FLAGS_MUST_CONFIRM',    128);
define('USER_FLAGS_NO_WALL_POSTS',   256);
define('USER_FLAGS_MUST_APPROVE',    512); // @deprecated

define('FIELD_FLAGS_MULTI_LINE',  1);
define('FIELD_FLAGS_LINK_URL',    2);
define('FIELD_FLAGS_ON_REGISTER', 4);

// how many seconds a form is valid for submission
define('FORM_EXPIRY_SECS', 86400);
define('FORM_KEY_LENGTH', 32);
...
if (FINAL_EXTERNAL_USERS) {
	...
	if (defined('FINAL_WORDPRESS_INTEGRATE_PATH')) {
		require_once INCLUDE_DIR . 'util/external-users-wp.php';
	} elseif (defined('FINAL_JOOMLA_INTEGRATE_PATH')) {
		require_once INCLUDE_DIR . 'util/external-users-joomla.php';
	} else {
		require_once EXTERNAL_DIR . 'external-users.php';
	}
*/
...
/*
	function qa_get_logged_in_user_cache()
	{
		...
		if (!isset($qa_cached_logged_in_user)) {
			...
			if (isset($user)) {
				$user['flags'] = isset($user['blocked']) ? USER_FLAGS_USER_BLOCKED : 0;
...
*/
	...
	function qa_session_var_suffix()
	{
		...
		if (!$qa_session_suffix) {
			$prefix = defined('USERS_PREFIX') ? USERS_PREFIX : TABLE_PREFIX;
			$qa_session_suffix = md5(DB_HOST . '/' . DB_USER . '/' . DB_PASS . '/' . DB_NAME . '/' . $prefix);
		}
	...
	function qa_session_verify_code($userid)
	{
		...
		return sha1(DB_HOST . '/' . DB_NAME . '/' . $userid);
	}
	...
	function qa_set_session_cookie($handle, $sessioncode, $remember)
	{
		...
		setcookie('qa_session', $handle . '/' . $sessioncode . '/' . ($remember ? 1 : 0), $remember ? (time() + 2592000) : 0, '/', COOKIE_DOMAIN, (bool)ini_get('session.cookie_secure'), true);
	}
	...
	function qa_clear_session_cookie()
	{
		...
		setcookie('qa_session', false, 0, '/', COOKIE_DOMAIN, (bool)ini_get('session.cookie_secure'), true);
	}
		...
		if ($countusers) // user exists so log them in
			...
			if (count($users) == 1) {
				...
				$userid = qa_create_new_user((string)@$fields['email'], null /* no password */, $handle,
					isset($fields['level']) ? $fields['level'] : USER_LEVEL_BASIC, @$fields['confirmed']);
...
/*
		...
		return qa_path('image', $params, $rootUrl, URL_FORMAT_PARAMS);
*/
		$blob = qa_db_read_one_assoc(qa_db_query_sub(
		...
		$path = substr($created, 0, 4) . '/' . substr($created, 5, 2) . '/' . substr($created, 8, 2);
		$path = '/upload/' . $path . '/' . $blobId . '.' . $format;
	...
	function qa_get_user_avatar_source($flags, $email, $blobId)
	{
		...
		if (qa_opt('avatar_allow_gravatar') && (($flags & USER_FLAGS_SHOW_GRAVATAR) > 0) && isset($email)) {
			return 'gravatar';
		} elseif (qa_opt('avatar_allow_upload') && (($flags & USER_FLAGS_SHOW_AVATAR) > 0) && isset($blobId)) {
			return 'local-user';
	...
	function qa_user_level_string($level)
	{
		...
		if ($level >= USER_LEVEL_SUPER)
			$string = 'users/level_super';
		elseif ($level >= USER_LEVEL_ADMIN)
			$string = 'users/level_admin';
		elseif ($level >= USER_LEVEL_MODERATOR)
			$string = 'users/level_moderator';
		elseif ($level >= USER_LEVEL_EDITOR)
			$string = 'users/level_editor';
		elseif ($level >= USER_LEVEL_EXPERT)
			$string = 'users/level_expert';
		elseif ($level >= USER_LEVEL_APPROVED)
...
} // end of: if (FINAL_EXTERNAL_USERS) { ... } else { ... }
...
{
	return qa_get_logged_in_user_field(FINAL_EXTERNAL_USERS ? 'publicusername' : 'handle');
}
...
/**
 * Return flags (seeUSER_FLAGS_*) of currently logged in user, or null if none
 */
function qa_get_logged_in_flags()
{
	if (FINAL_EXTERNAL_USERS)
		return qa_get_logged_in_user_field('blocked') ? USER_FLAGS_USER_BLOCKED : 0;
	else
 ...
 * @param int $userlevel AUSER_LEVEL_* constant to consider the user at a different level to usual (e.g. if
...
function qa_user_permit_error($permitoption = null, $limitaction = null, $userlevel = null, $checkblocks = true, $userfields = null)
{
	...
	if (!$error && isset($userid) && ($flags & USER_FLAGS_MUST_CONFIRM) && qa_opt('confirm_user_emails'))
		$error = 'confirm';
...
function qa_permit_error($permitoption, $userid, $userlevel, $userflags, $userpoints = null)
{
	...
	$permit = isset($permitoption) ? qa_opt($permitoption) :PERMIT_ALL;

	if (isset($userid) && ($permit ==PERMIT_POINTS || $permit ==PERMIT_POINTS_CONFIRMED || $permit ==PERMIT_APPROVED_POINTS)) {
		...
		if ($userpoints >= qa_opt($permitoption . '_points')) {
			$permit = $permit ==PERMIT_APPROVED_POINTS
				?PERMIT_APPROVED
				: ($permit ==PERMIT_POINTS_CONFIRMED ?PERMIT_CONFIRMED :PERMIT_USERS); // convert if user has enough points
		} else
			$permit =PERMIT_EXPERTS; // otherwise show a generic message so they're not tempted to collect points just for this
	}
...
function qa_permit_value_error($permit, $userid, $userlevel, $userflags)
{
	...
	if (!isset($userid) && $permit <PERMIT_ALL)
		return 'login';

	$levelError =
		($permit <=PERMIT_SUPERS && $userlevel < USER_LEVEL_SUPER) ||
		($permit <=PERMIT_ADMINS && $userlevel < USER_LEVEL_ADMIN) ||
		($permit <=PERMIT_MODERATORS && $userlevel < USER_LEVEL_MODERATOR) ||
		($permit <=PERMIT_EDITORS && $userlevel < USER_LEVEL_EDITOR) ||
		($permit <=PERMIT_EXPERTS && $userlevel < USER_LEVEL_EXPERT);

	if ($levelError)
		return 'level';

	if (isset($userid) && ($userflags & USER_FLAGS_USER_BLOCKED))
		return 'userblock';

	if ($permit >=PERMIT_USERS)
		return false;

	if ($permit >=PERMIT_CONFIRMED) {
		$confirmed = ($userflags & USER_FLAGS_EMAIL_CONFIRMED);
		// not currently supported by single sign-on integration; approved users and above don't need confirmation
		if (!FINAL_EXTERNAL_USERS && qa_opt('confirm_user_emails') && $userlevel < USER_LEVEL_APPROVED && !$confirmed) {
			return 'confirm';
		}
	} elseif ($permit >=PERMIT_APPROVED) {
		// check user is approved, only if we require it
		if (qa_opt('moderate_users') && $userlevel < USER_LEVEL_APPROVED) {
			return 'approve';
		}
	}
...
/**
 * Return whether a captcha is required for posts submitted by the current user. You can pass in aUSER_LEVEL_*
...
function qa_user_captcha_reason($userlevel = null)
{
	...
	if ($userlevel < USER_LEVEL_APPROVED) { // approved users and above aren't shown captchas
		$userid = qa_get_logged_in_userid();
		...
		elseif (qa_opt('moderate_users') && qa_opt('captcha_on_unapproved'))
			$reason = 'approve';
		elseif (qa_opt('confirm_user_emails') && qa_opt('captcha_on_unconfirmed') && !(qa_get_logged_in_flags() & USER_FLAGS_EMAIL_CONFIRMED))
			$reason = 'confirm';
	}
 ...
 *USER_LEVEL_* constant in $userlevel to consider the user at a different level to usual.
 * @param $userlevel
 * @return bool|mixed
 */
...
/**
 * Return whether moderation is required for posts submitted by the current user. You can pass in aUSER_LEVEL_*
 * constant in $userlevel to consider the user at a different level to usual (e.g. if they are performing this action
 * in a category for which they have elevated privileges).
 *
 * Possible results:
...
function qa_user_moderation_reason($userlevel = null)
{
	...
	if ($userlevel < USER_LEVEL_EXPERT && qa_user_permit_error('permit_moderate')) {
		// experts and above aren't moderated; if the user can approve posts, no point in moderating theirs
		$userid = qa_get_logged_in_userid();

		if (isset($userid)) {
			if (qa_opt('moderate_users') && qa_opt('moderate_unapproved') && ($userlevel < USER_LEVEL_APPROVED))
				$reason = 'approve';
			elseif (qa_opt('confirm_user_emails') && qa_opt('moderate_unconfirmed') && !(qa_get_logged_in_flags() & USER_FLAGS_EMAIL_CONFIRMED))
				$reason = 'confirm';
...
function qa_set_form_security_key()
{
	...
	if (!qa_is_logged_in() && !@$qa_form_key_cookie_set) {
		...
		if (strlen(@$_COOKIE['qa_key']) != FORM_KEY_LENGTH) {
			require_once INCLUDE_DIR . 'util/string.php';
			$_COOKIE['qa_key'] = qa_random_alphanum(FORM_KEY_LENGTH);
		}

		setcookie('qa_key', $_COOKIE['qa_key'], time() + 2 * FORM_EXPIRY_SECS, '/', COOKIE_DOMAIN, (bool)ini_get('session.cookie_secure'), true); // extend on every page request
	}
}
 ...
 *FORM_EXPIRY_SECS of $timestamp (in unix seconds) by the current user.
 * @param $action
 * @param $timestamp
 * @return mixed|string
 */
 ...
 *FORM_EXPIRY_SECS of now by the current user.
 * @param $action
 * @return mixed|string
 */
 ...
 * that the code has not expired (if more thanFORM_EXPIRY_SECS have passed). Logs causes for suspicion.
 * @param $action
 * @param $value
 * @return bool
 */
function qa_check_form_security_code($action, $value)
{
	...
	if (!empty($reportproblems) && DEBUG_PERFORMANCE) {
		@error_log(
			'PHP Tabloid form security violation for ' . $action .
