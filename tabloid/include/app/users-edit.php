...
/*
	User management (application level) for creating/modifying users
*/
...
function qa_handle_make_valid($handle)
{
	...
	for ($attempt = 0; $attempt <= 99; $attempt++) {
		...
		$tryhandle = qa_substr($handle, 0, DB_MAX_HANDLE_LENGTH - strlen($suffix)) . $suffix;
...
function qa_password_validate($password, $olduser = null)
{
	...
	if (!isset($error)) {
		$minpasslen = max(MIN_PASSWORD_LEN, 1);
 ...
 * @return mixed
 */
function qa_create_new_user($email, $password, $handle, $level = USER_LEVEL_BASIC, $confirmed = false)
{
	...
	if ($confirmed)
		qa_db_user_set_flag($userid, USER_FLAGS_EMAIL_CONFIRMED, true);

	if (qa_opt('show_notice_welcome'))
		qa_db_user_set_flag($userid, USER_FLAGS_WELCOME_NOTICE, true);

	$custom = qa_opt('show_custom_welcome') ? trim(qa_opt('custom_welcome')) : '';

	if (qa_opt('confirm_user_emails') && $level < USER_LEVEL_EXPERT && !$confirmed) {
		...
		if (qa_opt('confirm_user_required'))
			qa_db_user_set_flag($userid, USER_FLAGS_MUST_CONFIRM, true);

	} else
		$confirm = '';

	// we no longer use the 'approve_user_required' option to setUSER_FLAGS_MUST_APPROVE; this can be handled by the Permissions settings

	send_notification($userid, $email, $handle, qa_lang('emails/welcome_subject'), qa_lang('emails/welcome_body'), array(
...
function qa_send_new_confirm($userid)
{
	...
	if (!send_notification($userid, $userinfo['email'], $userinfo['handle'], qa_lang('emails/confirm_subject'), qa_lang('emails/confirm_body'), array(
...
function qa_complete_confirm($userid, $email, $handle)
{
	...
	qa_db_user_set_flag($userid, USER_FLAGS_EMAIL_CONFIRMED, true);
	qa_db_user_set_flag($userid, USER_FLAGS_MUST_CONFIRM, false);
...
/**
 * Set the user level of user $userid with $handle to $level (one of theUSER_LEVEL_* constraints in /qa-include/app/users.php)
...
function qa_set_user_level($userid, $handle, $level, $oldlevel)
{
	...
	if ($level >= USER_LEVEL_APPROVED) {
		// no longer necessary asUSER_FLAGS_MUST_APPROVE is deprecated, but kept for posterity
		qa_db_user_set_flag($userid, USER_FLAGS_MUST_APPROVE, false);
	}
...
function qa_set_user_blocked($userid, $handle, $blocked)
{
	...
	qa_db_user_set_flag($userid, USER_FLAGS_USER_BLOCKED, $blocked);
	qa_db_uapprovecount_update();
...
function qa_start_reset_user($userid)
{
	...
	if (!send_notification($userid, $userinfo['email'], $userinfo['handle'], qa_lang('emails/reset_subject'), qa_lang('emails/reset_body'), array(
...
function qa_complete_reset_user($userid)
{
	...
	$password = qa_random_alphanum(max(MIN_PASSWORD_LEN, NEW_PASSWORD_LEN));

	$userinfo = qa_db_select_with_pending(qa_db_user_account_selectspec($userid, true));

	if (!send_notification($userid, $userinfo['email'], $userinfo['handle'], qa_lang('emails/new_password_subject'), qa_lang('emails/new_password_body'), array(
		'^password' => $password,
		'^url' => qa_opt('site_url'),
	))) {
...
function qa_set_user_avatar($userid, $imagedata, $oldblobid = null)
{
	...
	if (isset($imagedata)) {
		...
		if (isset($newblobid)) {
			...
			qa_db_user_set_flag($userid, USER_FLAGS_SHOW_AVATAR, true);
