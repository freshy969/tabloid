...
/*
	Common functions for creating theme-ready structures from data
*/

define('PAGE_FLAGS_EXTERNAL', 1);
define('PAGE_FLAGS_NEW_WINDOW', 2);
...
function qa_post_html_fields($post, $userid, $cookieid, $usershtml, $dummy, $options = array())
{
	...
	$isquestion = $post['basetype'] == 'Q' || $post['type'] == 'POST';
	...
	if (@$options['voteview']) {
		...
		if (array_key_exists('netvotes', $post)) {
			$netvotes = (int) $post['netvotes'];
		} else {
	...
	if (@$options['whatview']) {
		...
		if (@$options['whatlink'] && strlen(@$options['q_request'])) {
			$fields['what_url'] = ($post['basetype'] == 'Q' || $post['type'] == 'POST')
...
*/	
	// Date formatting is duplicated in theme-base.php:post_meta_when
	if (isset($post['created'])) {				

		$now = new DateTime();
		$date = (new DateTime())->setTimestamp($post['created']);
		$diff = date_diff($now, $date);

		$month = qa_lang_html('misc/month_' . $date->format('m'));
		
		if (!$diff->y) {
			if (!$diff->m && !$diff->d)
				$date = 'Сегодня ' . $date->format('H:i');
			else if (!$diff->m && $diff->d == 1)	 
				$date = 'Вчера ' . $date->format('H:i');
			else	
				$date = $date->format('d') . ' ' . $month;				
		} else {	
			$date = $date->format('d') . ' ' . $month . ' ' . $date->format('Y');
		}
		
		$fields['when']['data'] = $date;
		if ($microdata) {
			$gmdate = gmdate('Y-m-d\TH:i:sO', $post['created']);
			$fields['when']['data'] = '<time itemprop="dateCreated" datetime="' . $gmdate . '" title="' . $gmdate . '">' . $fields['when']['data'] . '</time>';
		}
	...
	if (@$options['updateview'] && isset($post['updated']) &&
		($post['updatetype'] != UPDATE_SELECTED || $isselected) && // only show selected change if it's still selected
		( // otherwise check if one of these conditions is fulfilled...
			...
			($post['hidden'] && ($post['updatetype'] == UPDATE_VISIBLE)) || // ... the post was hidden as the last action
			(qa_post_is_closed($post) && $post['updatetype'] == UPDATE_CLOSED) || // ... the post was closed as the last action
...
function qa_other_to_q_html_fields($question, $userid, $cookieid, $usershtml, $dummy, $options)
{
    ...
    // Add POST type here
	if ($question['obasetype'] != 'Q' || @$question['oupdatetype'] == UPDATE_FOLLOWS)
 ...
 * incorrect URLs from (http://www.tabloid.dev) but allow URLs such as http://www.wikipedia.org/Computers_(Software)
 * @param $html
...
function qa_html_page_links($request, $start, $pagesize, $count, $prevnext, $params = array(), $hasmore = false, $anchor = null)
{
	...
	$lastpage = ceil(min((int)$count, 1 + MAX_LIMIT_START) / $pagesize);
...
function qa_users_sub_navigation()
{
	...
	$moderatorPlus = qa_get_logged_in_level() >= USER_LEVEL_MODERATOR;
...
function qa_user_sub_navigation($handle, $selected, $ismyuser = false)
{
	...
	if (FINAL_EXTERNAL_USERS || !$ismyuser)
		unset($navigation['account']);

	if (!$ismyuser)
		unset($navigation['favorites']);

	if (FINAL_EXTERNAL_USERS || !$ismyuser || !qa_opt('allow_private_messages') || !qa_opt('show_message_history'))
...
{
	return ($page['flags'] & PAGE_FLAGS_EXTERNAL)
...
function qa_navigation_add_page(&$navigation, $page)
{
	if (!isset($page['permit']) || !qa_permit_value_error($page['permit'], qa_get_logged_in_userid(), qa_get_logged_in_level(), qa_get_logged_in_flags())) {
		...
		$navigation[($page['flags'] & PAGE_FLAGS_EXTERNAL) ? ('custom-' . $page['pageid']) : ($page['tags'] . '$')] = array(
			'url' => qa_html($url),
			'label' => qa_html($page['title']),
			'opposite' => ($page['nav'] == 'O'),
			'target' => ($page['flags'] & PAGE_FLAGS_NEW_WINDOW) ? '_blank' : null,
			'selected' => ($page['flags'] & PAGE_FLAGS_EXTERNAL) && (($url == qa_path(qa_request())) || ($url == qa_self_html())),
		);
	}
}
...
function qa_set_up_category_field(&$qa_content, &$field, $fieldname, $navcategories, $categoryid, $allownone, $allownosub, $maxdepth = null, $excludecategoryid = null)
{
	...
	if (isset($maxdepth))
		$maxdepth = min(CATEGORY_DEPTH, $maxdepth);
	else
		$maxdepth = CATEGORY_DEPTH;
    ...
    //$field['tags'] = sprintf('name="%s_0" id="%s_0" onchange="qa_category_select(%s);" style="width: 100%;"', $fieldname, $fieldname, qa_js($fieldname));
	$field['options'] = array();
...
function qa_set_up_type_field(&$qa_content, &$field, $fieldname, $navcategories, $categoryid, $allownone, $allownosub, $maxdepth = null, $excludecategoryid = null)
{
	//$pathcategories = qa_category_path($navcategories, $categoryid);
	...
	$field['type'] = 'select';
	//$field['tags'] = sprintf('name="%s_0" id="%s_0" onchange="qa_category_select(%s);"', $fieldname, $fieldname, qa_js($fieldname));
    $field['tags'] = sprintf('name="%s_0" id="%s_0"', $fieldname, $fieldname);
	$field['options'] = [];
...
{
	for ($level = CATEGORY_DEPTH; $level >= 1; $level--) {
...
function qa_load_theme_class($theme, $template, $content, $request)
{
	...
	require_once INCLUDE_DIR . 'theme-base.php';

	$classname = 'html_theme_base';

	// Then load the selected theme if valid, otherwise load the Classic theme

	if (!file_exists(THEMES_DIR . $theme . '/styles.css'))
		$theme = 'zen';

	// $themeroothtml = qa_html(qa_path_to_root() . 'qa-theme/' . $theme . '/');
	$themeroothtml = qa_html(qa_path_to_root() . 'themes/' . $theme . '/');

	if (file_exists(THEMES_DIR . $theme . '/theme.php')) {
		require_once THEMES_DIR . $theme . '/theme.php';

		if (class_exists('html_theme'))
			$classname = 'html_theme';
	}
	...
	if (!qa_user_maximum_permit_error('permit_view_voters_flaggers')) {
		$loadlayers[] = array(
			'directory' => INCLUDE_DIR . 'plugins/',
			'include' => 'layer-voters-flaggers.php',
			'urltoroot' => null,
		);
	}
	...
	foreach ($loadlayers as $layer) {
		...
		if (strlen($layerphp)) {
			...
			if (preg_match('/\s+class\s+html_theme_layer\s+extends\s+html_theme_base\s+/im', $layerphp) != 1)
				qa_fatal_error('Class for layer must be declared as "class html_theme_layer extends html_theme_base" in ' . $layer['directory'] . $layer['include']);

			$searchwordreplace = array(
				'html_theme_base::html_theme_base' => $classname . '::__construct', // PHP5 constructor fix
				'parent::html_theme_base' => 'parent::__construct', // PHP5 constructor fix
				'html_theme_layer' => $newclassname,
				'html_theme_base' => $classname,
				'HTML_THEME_LAYER_DIRECTORY' => "'" . $layer['directory'] . "'",
				'HTML_THEME_LAYER_URLTOROOT' => "'" . qa_path_to_root() . $layer['urltoroot'] . "'",
			);
