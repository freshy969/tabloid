...
/*
	Functions for sending a mailing to all users
*/
...
function qa_mailing_perform_step()
{
	...
	if (count($users)) {
		...
		foreach ($users as $user) {
			if (($user['flags'] & USER_FLAGS_NO_MAILINGS) || // exclude users who don't want to get the mailings
				($user['flags'] & USER_FLAGS_USER_BLOCKED) || // exclude blocked users
				($isModeratingUsers && ($user['level'] < USER_LEVEL_APPROVED))) { // if moderating users exclude unapproved users
				continue;
			}
