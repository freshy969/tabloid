...
/*
	Higher-level functions to create and manipulate posts
*/
...
function qa_post_create($type, $parentid, $title, $content, $format = '', $categoryid = null, $tags = null, $userid = null,
    ...
    $type2 = null, $image = null, $lead = null) 
{
...
function qa_post_set_category($postid, $categoryid, $byuserid = null)
{
	...
	if ($oldpost['basetype'] == 'Q' || $oldpost['basetype'] == 'P') {
		$byhandle = qa_userid_to_handle($byuserid);
...
{
	qa_post_set_status($postid, $hidden ? POST_STATUS_HIDDEN : POST_STATUS_NORMAL, $byuserid);
}


/**
 * Change the status of $postid to $status, which should be one of thePOST_STATUS_* constants defined in
...
function qa_post_delete($postid)
{
	...
	if (!$oldpost['hidden']) {
		qa_post_set_status($postid, POST_STATUS_HIDDEN, null);
		$oldpost = qa_post_get_full($postid, 'QAC');
	}
...
function qa_post_get_question_commentsfollows($questionid)
{
	...
	foreach ($achildposts as $postid => $post) {
		if ($post['basetype'] == 'Q' || $post['basetype'] == 'P' || $post['basetype'] == 'C')
			$commentsfollows[$postid] = $post;
	}
...
{
	if ($parent['basetype'] == 'Q' || $parent['basetype'] == 'P')
		$question = $parent;
	else
		$question = qa_post_get_full($parent['parentid'], $parent['basetype']);
