...
/*
	Getting and setting admin options (application level)
*/

require_once INCLUDE_DIR . 'db/options.php';

define('PERMIT_ALL', 150);
define('PERMIT_USERS', 120);
define('PERMIT_CONFIRMED', 110);
define('PERMIT_POINTS', 106);
define('PERMIT_POINTS_CONFIRMED', 104);
define('PERMIT_APPROVED', 103);
define('PERMIT_APPROVED_POINTS', 102);
define('PERMIT_EXPERTS', 100);

define('PERMIT_COMPANIES', 80);

define('PERMIT_EDITORS', 70);
define('PERMIT_MODERATORS', 40);

define('PERMIT_ADMINS', 20);
define('PERMIT_SUPERS', 0);
 ...
 * From Q2A 1.8 we always load the options in a separate query regardless ofOPTIMIZE_DISTANT_DB.
 */
...
function qa_default_option($name)
{
	...
	$fixed_defaults = array(
		...
		'page_size_wall' => 10,
		'pages_prev_next' => 3,
		'permit_anon_view_ips' =>PERMIT_EDITORS,
		'permit_close_q' =>PERMIT_EDITORS,
		'permit_delete_hidden' =>PERMIT_MODERATORS,
		'permit_edit_a' =>PERMIT_EXPERTS,
		'permit_edit_c' =>PERMIT_EDITORS,
		'permit_edit_q' =>PERMIT_EDITORS,
		'permit_edit_silent' =>PERMIT_MODERATORS,
		'permit_flag' =>PERMIT_CONFIRMED,
		'permit_hide_show' =>PERMIT_EDITORS,
		'permit_moderate' =>PERMIT_EXPERTS,
		'permit_post_wall' =>PERMIT_CONFIRMED,
		'permit_select_a' =>PERMIT_EXPERTS,
		'permit_view_q_page' =>PERMIT_ALL,
		'permit_view_new_users_page' =>PERMIT_EDITORS,
		'permit_view_special_users_page' =>PERMIT_MODERATORS,
		'permit_view_voters_flaggers' =>PERMIT_ADMINS,
		'permit_vote_a' =>PERMIT_USERS,
		'permit_vote_c' =>PERMIT_USERS,
		'permit_vote_down' =>PERMIT_USERS,
		'permit_vote_q' =>PERMIT_USERS,
	...
	switch ($name) {
		...
		case 'permit_post_q': // convert from deprecated option if available
			$value = qa_opt('ask_needs_login') ?PERMIT_USERS :PERMIT_ALL;
			break;

		case 'permit_post_a': // convert from deprecated option if available
			$value = qa_opt('answer_needs_login') ?PERMIT_USERS :PERMIT_ALL;
			break;

		case 'permit_post_c': // convert from deprecated option if available
			$value = qa_opt('comment_needs_login') ?PERMIT_USERS :PERMIT_ALL;
			break;
...
function qa_post_html_defaults($basetype, $full = false)
{
	...
	return array(
		...
		'tagsview' => ($basetype == 'Q' || $basetype == 'P') && qa_using_tags(),
		'categoryview' => ($basetype == 'Q' || $basetype == 'P') && qa_using_categories(),
		'contentview' => $full,
		'voteview' => qa_get_vote_view($basetype, $full),
		'flagsview' => qa_opt('flagging_of_posts') && $full,
		'favoritedview' => true,
		'answersview' => $basetype == 'Q' || $basetype == 'P',
		'viewsview' => ($basetype == 'Q' || $basetype == 'P') && qa_opt('do_count_q_views') && ($full ? qa_opt('show_view_count_q_page') : qa_opt('show_view_counts')),
		'whatview' => true,
...
//	if (!$enabledif || ($basetype == 'Q' && !$full && qa_opt('voting_on_q_page_only'))) {
	if (!$enabledif || (($basetype == 'Q' || $basetype == 'P') && !$full && qa_opt('voting_on_q_page_only'))) {
...
/**
 * Returns true if the home page has been customized, either due to admin setting, orCONST_PATH_MAP
 */
