...
/*
	Application-level file upload functionality
*/
 ...
 * @return array
 */
function qa_upload_file($localfilename, $sourcefilename, $maxfilesize = null, $onlyimage = false, $imagemaxwidth = null, $imagemaxheight = null, $created = null)
{
	if (qa_to_override(__FUNCTION__)) { $args=func_get_args(); return qa_call_override(__FUNCTION__, $args); }
...
if (qa_get_logged_in_level() < USER_LEVEL_EDITOR) {
	...
	switch (qa_user_permit_error(null,LIMIT_UPLOADS)) {
		...
		case false:
			qa_limits_increment(qa_get_logged_in_userid(),LIMIT_UPLOADS);
			break;
	...
	$result['blobid'] = qa_create_blob($content, $format, $sourcefilename, $userid, $cookieid, qa_remote_ip_address(), $created);
...
/*
    ...
    $blobcreated = qa_db_read_one_value(qa_db_query_sub('SELECT created FROM `^blobs` WHERE `blobid` = #', $blobid), true);
    $blobpath = substr($blobcreated, 0, 4).'/'.substr($blobcreated, 5, 2).'/'.substr($blobcreated, 8, 2);
    //$path = rtrim(BLOBS_DIRECTORY, '/').'/'.$blobpath;
    $result['bloburl'] = $blobpath . $result['blobid'] . '.' . $result['format'];
*/
	return $result;
}
 ...
 * @return array
 */
function qa_upload_file_one($maxfilesize = null, $onlyimage = false, $imagemaxwidth = null, $imagemaxheight = null, $created = null)
{
	$file = reset($_FILES);
	return qa_upload_file($file['tmp_name'], $file['name'], $maxfilesize, $onlyimage, $imagemaxwidth, $imagemaxheight, $created);
}
