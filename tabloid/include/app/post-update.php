...
/*
	Changing questions, answer and comments (application level)
*/
...
require_once INCLUDE_DIR . 'db/post-update.php';
require_once INCLUDE_DIR . 'db/points.php';
require_once INCLUDE_DIR . 'db/hotness.php';


define('POST_STATUS_NORMAL', 0);
define('POST_STATUS_HIDDEN', 1);
define('POST_STATUS_QUEUED', 2);
...
function qa_question_set_content($oldquestion, $title, $content, $format, $text, $tagstring, $notify, $userid, $handle, $cookieid,
    $extravalue = null, $name = null, $remoderate = false, $silent = false,
    $type = null, $image = null, $lead = null) 
{
    ...
    $typechanged = strcmp($oldquestion['type'], $type) !== 0;
    $imagechanged = strcmp($oldquestion['image'], $image) !== 0;
    $leadchanged = strcmp($oldquestion['lead'], $lead) !== 0;

    $setupdated = ($titlechanged || $contentchanged || $tagschanged || $typechanged || $imagechanged || $leadchanged) && (!$wasqueued) && !$silent;
	...
	qa_db_post_set_content($oldquestion['postid'], $title, $content, $format, $tagstring, $notify,
		...
		($titlechanged || $contentchanged || $typechanged || $imagechanged || $leadchanged) ? UPDATE_CONTENT : UPDATE_TAGS, $name, 
        $type, $image, $lead); 

	if (isset($extravalue)) {
		require_once INCLUDE_DIR . 'db/metas.php';
	...
	if ($setupdated && $remoderate) {
        ...
        // TODO Remoderate POST and Q correctly
	...
	} elseif ($oldquestion['type'] == 'Q' || $oldquestion['type'] == 'POST') { // not hidden or queued
        qa_post_index($oldquestion['postid'], $type, $oldquestion['postid'], $oldquestion['parentid'], $title, $content, $format, $text, $tagstring, @$oldquestion['categoryid'], $type, $image, $lead);
		if ($tagschanged) {
...
function qa_question_close_other($oldquestion, $oldclosepost, $note, $userid, $handle, $cookieid)
{
	...
	if ($oldquestion['type'] == 'Q' || $oldquestion['type'] == 'POST')
...
{
	qa_question_set_status($oldquestion, $hidden ? POST_STATUS_HIDDEN : POST_STATUS_NORMAL, $userid, $handle, $cookieid, $answers, $commentsfollows, $closepost);
}


/**
 * Set the status (application level) of $oldquestion to $status, one of thePOST_STATUS_* constants above. Pass
...
function qa_question_set_status($oldquestion, $status, $userid, $handle, $cookieid, $answers, $commentsfollows, $closepost = null)
{
	...
	} elseif ($status == POST_STATUS_NORMAL) {
		$newtype = 'Q';
		if ($wasqueued)
			$event = 'q_approve';
		elseif ($washidden) {
	...
	qa_db_post_set_type($oldquestion['postid'], $newtype, $setupdated ? $userid : null, $setupdated ? qa_remote_ip_address() : null, UPDATE_VISIBLE);

	if ($wasqueued && $status == POST_STATUS_NORMAL && qa_opt('moderate_update_time')) { // ... for approval of a post, can set time to now instead
	...
	if ($wasqueued || ($status == POST_STATUS_QUEUED))
		qa_db_queuedcount_update();

	if ($oldquestion['flagcount'])
		qa_db_flaggedcount_update();

    // TODO Work with POST type
	if ($status == POST_STATUS_NORMAL) {
	...
	if ($wasqueued && ($status == POST_STATUS_NORMAL) && !$wasrequeued) {
		...
		qa_report_event('q_post', $oldquestion['userid'], $oldquestion['handle'], $oldquestion['cookieid'], $eventparams + array(
			...
			'email' => email_validate($oldquestion['notify']) ? $oldquestion['notify'] : null,
...
function qa_answer_set_content($oldanswer, $content, $format, $text, $notify, $userid, $handle, $cookieid, $question, $name = null, $remoderate = false, $silent = false)
{
	...
	qa_db_post_set_content($oldanswer['postid'], $oldanswer['title'], $content, $format, $oldanswer['tags'], $notify,
		$setupdated ? $userid : null, $setupdated ? qa_remote_ip_address() : null, UPDATE_CONTENT, $name);
	...
	} elseif ($oldanswer['type'] == 'A' && ($question['type'] == 'Q' || $question['type'] == 'POST')) { // don't index if question or answer are hidden/queued
...
{
	qa_answer_set_status($oldanswer, $hidden ? POST_STATUS_HIDDEN : POST_STATUS_NORMAL, $userid, $handle, $cookieid, $question, $commentsfollows);
}


/**
 * Set the status (application level) of $oldanswer to $status, one of thePOST_STATUS_* constants above. Pass
...
function qa_answer_set_status($oldanswer, $status, $userid, $handle, $cookieid, $question, $commentsfollows)
{
	...
	qa_db_post_set_type($oldanswer['postid'], $newtype, $setupdated ? $userid : null, $setupdated ? qa_remote_ip_address() : null, UPDATE_VISIBLE);
	...
	if (($question['type'] == 'Q' || $question['type'] == 'POST') && $status == POST_STATUS_NORMAL) { // even if answer visible, don't index if question is hidden or queued
	...
	if ($wasqueued && ($status == POST_STATUS_NORMAL) && !$wasrequeued) {
		...
		qa_report_event('a_post', $oldanswer['userid'], $oldanswer['handle'], $oldanswer['cookieid'], $eventparams + array(
			...
			'email' => email_validate($oldanswer['notify']) ? $oldanswer['notify'] : null,
...
function qa_comment_set_content($oldcomment, $content, $format, $text, $notify, $userid, $handle, $cookieid, $question, $parent, $name = null, $remoderate = false, $silent = false)
{
	...
	} elseif ($oldcomment['type'] == 'C' && ($question['type'] == 'Q' || $question['type'] == 'POST') && ($parent['type'] == 'Q' || $parent['type'] == 'POST' || $parent['type'] == 'A')) { // all must be visible
...
function qa_answer_to_comment($oldanswer, $parentid, $content, $format, $text, $notify, $userid, $handle, $cookieid, $question, $answers, $commentsfollows, $name = null, $remoderate = false, $silent = false)
{
	...
	qa_db_post_set_type($oldanswer['postid'], $newtype, ($wasqueued || $silent) ? null : $userid,
		($wasqueued || $silent) ? null : qa_remote_ip_address(), UPDATE_TYPE);
	...
	} elseif ($oldanswer['type'] == 'A' && ($question['type'] == 'Q' || $question['type'] == 'POST') && ($parent['type'] == 'Q' || $parent['type'] == 'POST' || $parent['type'] == 'A')) // only if all fully visible
...
{
	qa_comment_set_status($oldcomment, $hidden ? POST_STATUS_HIDDEN : POST_STATUS_NORMAL, $userid, $handle, $cookieid, $question, $parent);
}


/**
 * Set the status (application level) of $oldcomment to $status, one of thePOST_STATUS_* constants above. Pass the
...
function qa_comment_set_status($oldcomment, $status, $userid, $handle, $cookieid, $question, $parent)
{
	...
	qa_db_post_set_type($oldcomment['postid'], $newtype, $setupdated ? $userid : null, $setupdated ? qa_remote_ip_address() : null, UPDATE_VISIBLE);
	...
	if (($question['type'] == 'Q' || $question['type'] == 'POST') && ($parent['type'] == 'Q' || $parent['type'] == 'POST' || $parent['type'] == 'A') && $status == POST_STATUS_NORMAL) {
	...
	if ($wasqueued && $status == POST_STATUS_NORMAL && !$wasrequeued) {
		...
		qa_report_event('c_post', $oldcomment['userid'], $oldcomment['handle'], $oldcomment['cookieid'], $eventparams + array(
			...
			'email' => email_validate($oldcomment['notify']) ? $oldcomment['notify'] : null,
