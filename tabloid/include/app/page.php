...
/*
	Routing and utility functions for page requests
*/
...
{
	if (!FINAL_EXTERNAL_USERS && !qa_is_logged_in()) {
				...
				if (isset($noticeid)) {
					if (!qa_check_form_security_code('notice-' . $noticeid, qa_post_text('code')))
						...
						elseif ($noticeid == 'welcome') {
							...
							qa_db_user_set_flag(qa_get_logged_in_userid(), USER_FLAGS_WELCOME_NOTICE, false);
...
function qa_get_request_content()
{
	...
	if (isset($routing[$requestlower])) {
		...
		$qa_content = require INCLUDE_DIR . $routing[$requestlower];

	} elseif (isset($routing[$firstlower . '/'])) {
		qa_set_template($firstlower);
		$qa_content = require INCLUDE_DIR . $routing[$firstlower . '/'];

	} elseif (is_numeric($requestparts[0])) {
		qa_set_template('question');
		$qa_content = require INCLUDE_DIR . 'pages/question.php';

	} else {
		qa_set_template(strlen($firstlower) ? $firstlower : 'qa'); // will be changed later
		$qa_content = require INCLUDE_DIR . 'pages/default.php'; // handles many other pages, including custom pages and page modules
	}

	if ($firstlower == 'admin') {
		$_COOKIE['qa_admin_last'] = $requestlower; // for navigation tab now...
		setcookie('qa_admin_last', $_COOKIE['qa_admin_last'], 0, '/', COOKIE_DOMAIN, (bool)ini_get('session.cookie_secure'), true); // ...and in future
	}
...
function qa_output_content($qa_content)
{
	...
	if (isset($userid) && $requestlower != 'confirm' && $requestlower != 'account') {
		...
		if (($flags & USER_FLAGS_MUST_CONFIRM) && !($flags & USER_FLAGS_EMAIL_CONFIRMED) && qa_opt('confirm_user_emails')) {
			$qa_content = qa_content_prepare();
	...
	if (isset($qa_content['script_rel'])) {
		foreach ($qa_content['script_rel'] as $script_rel) {
			$script[] = '<script src="' . qa_html($script_rel) . '"></script>';
		}
	}
...
function qa_content_prepare($voting = false, $categoryids = array())
{
	...
	if (qa_get_logged_in_level() >= USER_LEVEL_ADMIN || !qa_user_maximum_permit_error('permit_moderate') ||
	...
	if (qa_is_logged_in()) {
		$qa_content['loggedin'] = qa_lang_html_sub_split('main/logged_in_x', FINAL_EXTERNAL_USERS
	...
	if (FINAL_EXTERNAL_USERS || !qa_is_logged_in()) {
		...
		setcookie('qa_noticed', 1, time() + 86400 * 3650, '/', COOKIE_DOMAIN, (bool)ini_get('session.cookie_secure'), true); // don't show first-time notice if a user has logged in

		if (qa_opt('show_notice_welcome') && (qa_get_logged_in_flags() & USER_FLAGS_WELCOME_NOTICE)) {
...
//	$qa_content['script_rel'] = [ '/js/jquery-3.3.1.min.js');
//	$qa_content['script_rel'][] = '/js/global.js?' .VERSION;

	$qa_content['script_rel'] = [ 
		//'/js/jquery-3.3.1.min.js',
		'/js/jquery.js',
		//'/js/global.js?' .VERSION,
		'/js/global.js',
	];
...
{
	return min(max(0, (int)qa_get('start')), MAX_LIMIT_START);
}
