...
/*
	Handling private or public messages (wall posts)
*/
...
function qa_wall_error_html($fromuserid, $touserid, $touserflags)
{
	...
	if (!FINAL_EXTERNAL_USERS && qa_opt('allow_user_walls')) {
		if (($touserflags & USER_FLAGS_NO_WALL_POSTS) && !(isset($fromuserid) && $fromuserid == $touserid))
			return qa_lang_html('profile/post_wall_blocked');

		else {
			switch (qa_user_permit_error('permit_post_wall',LIMIT_WALL_POSTS)) {
				case 'limit':
