...
/*
	Managing database recalculations (clean-up operations) and status messages
*/
...
/*
	...
	^posts (catidpath1, catidpath2, catidpath3): hierarchical path to category ids (requiresCATEGORY_DEPTH=4)
...
require_once INCLUDE_DIR . 'db/recalc.php';
		...
		case 'doreindexcontent_pagereindex':
			...
			if (count($pages)) {
				...
				foreach ($pages as $pageid => $page) {
					if (!($page['flags'] & PAGE_FLAGS_EXTERNAL)) {
						...
						foreach ($comments as $comment) {
							foreach ($keyuserids as $keyuserid => $dummy) {
								if ($keyuserid != $comment['userid'] && $keyuserid != @$posts[$parentid]['userid']) {
									qa_db_event_create_not_entity($keyuserid, $questionid, $comment['postid'], UPDATE_FOLLOWS, $comment['userid'], $comment['created']);
								}
							}
...
function qa_recalc_transition(&$state, $operation)
{
	...
	$next = (FINAL_EXTERNAL_USERS && ($operation == 'dorecalcpoints_recalc')) ? '' : 0;
	$done = 0;
