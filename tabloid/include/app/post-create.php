...
/*
	Creating questions, answers and comments (application level)
*/
...
function qa_question_create($followanswer, $userid, $handle, $cookieid, $title, $content, $format, $text, $tagstring, $notify, $email,
	$categoryid = null, $extravalue = null, $queued = false, $name = null, $type = null, $image = null, $lead = null)
{
    ...
    if ($type && $type == 'POST')
        ...
        $type = $queued ? 'Q_QUEUED' : 'Q';

	$postid = qa_db_post_create($type, @$followanswer['postid'], $userid, isset($userid) ? null : $cookieid,
		qa_remote_ip_address(), $title, $content, $format, $tagstring, qa_combine_notify_email($userid, $notify, $email),
		$categoryid, isset($userid) ? null : $name, $image, $lead);
	...
	if ($queued) {
        ...
        qa_post_index($postid, $type, $postid, @$followanswer['postid'], $title, $content, $format, $text, $tagstring, $categoryid, $type, $image, $lead);
 ...
 * @param $categoryid
 */
function qa_post_index($postid, $type, $questionid, $parentid, $title, $content, $format, $text, $tagstring, $categoryid, $type2 = null, $image = null, $lead = null)
{
	global $qa_post_indexing_suspended;
	...
	foreach ($searches as $search)
		$search->index_post($postid, $type, $questionid, $parentid, $title, $content, $format, $text, $tagstring, $categoryid, $type2, $image, $lead);
}
...
function qa_answer_create($userid, $handle, $cookieid, $content, $format, $text, $notify, $email, $question, $queued = false, $name = null)
{
	...
	if ($queued) {
		...
		if ($question['type'] == 'Q' || $question['type'] == 'POST') // don't index answer if parent question is hidden or queued
...
function qa_comment_create($userid, $handle, $cookieid, $content, $format, $text, $notify, $email, $question, $parent, $commentsfollows, $queued = false, $name = null)
{
	...
	if ($queued) {
		...
		if (($question['type'] == 'Q' || $question['type'] == 'POST') && 
			($parent['type'] == 'Q' || $parent['type'] == 'A' || $parent['type'] == 'POST')) { 
