...
/*
	Builders of selectspec arrays (see db.php) used to specify database SELECTs
*/
...
function qa_db_posts_basic_selectspec($voteuserid = null, $full = false, $user = true)
{
	...
	$selectspec = array(
		'columns' => array(
// Zen 			'^posts.postid', '^posts.categoryid', '^posts.type', 'basetype' => 'LEFT(^posts.type, 1)',
			'^posts.postid', '^posts.type', 'basetype' => 'LEFT(^posts.type, 1)',
...
// Zen			'categoryname' => '^categories.title', 'categorybackpath' => "^categories.backpath",
// Zen			'categoryids' => "CONCAT_WS(',', ^posts.catidpath1, ^posts.catidpath2, ^posts.catidpath3, ^posts.categoryid)",

// Zen : adding new fields
            '^posts.image', '^posts.lead', '^posts.promo', '^posts.popularity', '^posts.categoryid'

		),

		'arraykey' => 'postid',

// Zen : disable categories
        'source' => '^posts',
...
/* Zen
	...
	if (isset($voteuserid)) {
		require_onceINCLUDE_DIR . 'app/updates.php';
		...
		array_push($selectspec['arguments'], $voteuserid, $voteuserid, ENTITY_QUESTION);
	}
*/
	if ($full) {
...
// Zen That is for Wordpress integration - usefulness!
...
function qa_db_categoryslugs_sql_args($categoryslugs, &$arguments)
{
	...
	if ($levels > 0 && $levels <= CATEGORY_DEPTH) {
		$arguments[] = qa_db_slugs_to_backpath($categoryslugs);
		return (($levels == CATEGORY_DEPTH) ? 'categoryid' : ('catidpath' . $levels)) . '=(SELECT categoryid FROM ^categories WHERE backpath=$ LIMIT 1) AND ';
	}
...
{    
	if ($specialtype == 'Q' || $specialtype == 'Q_QUEUED' || $specialtype == 'POST') {
		$type = $specialtype;
	} else {
		$type = $specialtype ? 'Q_HIDDEN' : 'Q'; // for backwards compatibility
	}

	$count = isset($count) ? min($count, DB_RETRIEVE_QS_AS) : DB_RETRIEVE_QS_AS;
...
SELECT qa_posts.postid, qa_posts.type, LEFT(qa_posts.type, 1) AS `basetype`, INSTR(qa_posts.type, '_HIDDEN')>0 AS `hidden`, INSTR(qa_posts.type, '_QUEUED')>0 AS `queued`,
qa_posts.acount, qa_posts.selchildid, qa_posts.closedbyid, qa_posts.upvotes, qa_posts.downvotes, qa_posts.netvotes, qa_posts.views, qa_posts.hotness, qa_posts.flagcount,
qa_posts.title, qa_posts.tags, UNIX_TIMESTAMP(qa_posts.created) AS `created`, qa_posts.name, qa_posts.image, qa_posts.lead, qa_posts.promo, qa_posts.popularity,
qa_posts.userid, qa_posts.cookieid, qa_posts.createip AS `createip`, qa_userpoints.points
FROM qa_posts
LEFT JOIN qa_userpoints ON qa_posts.userid=qa_userpoints.userid
JOIN
(SELECT postid FROM qa_posts WHERE (type='Q' OR type='POST') ORDER BY qa_posts.hotness DESC LIMIT 0,25) y
ON qa_posts.postid=y.postid;
...
SELECT
        qa_posts.postid, qa_posts.type, LEFT(qa_posts.type, 1) AS `basetype`,
        INSTR(qa_posts.type, '_HIDDEN')>0 AS `hidden`, INSTR(qa_posts.type, '_QUEUED')>0 AS `queued`,
        qa_posts.acount, qa_posts.selchildid, qa_posts.closedbyid, qa_posts.upvotes, qa_posts.downvotes, qa_posts.netvotes, qa_posts.views, qa_posts.hotness,
        qa_posts.flagcount, qa_posts.title, qa_posts.tags, UNIX_TIMESTAMP(qa_posts.created) AS `created`, qa_posts.name, qa_posts.image, qa_posts.lead,
        qa_posts.promo, qa_posts.popularity, qa_posts.userid, qa_posts.cookieid, qa_posts.createip AS `createip`, qa_userpoints.points

FROM qa_posts
LEFT JOIN qa_userpoints ON qa_posts.userid=qa_userpoints.userid
WHERE type='Q' OR type='POST'
ORDER BY qa_posts.created DESC
LIMIT 0,25;

*/

    // Zen Mogul Slow Version
/*
    $typesSQL = "type=$ ";
    if ($type == 'Q')
        $typesSQL = "(type=$ OR type='POST') ";

    $selectspec['source'] .=
		" JOIN (SELECT postid FROM ^posts WHERE " .
		qa_db_categoryslugs_sql_args($categoryslugs, $selectspec['arguments']) .
		(isset($createip) ? "createip=UNHEX($) AND " : "") .
		$typesSQL . $sortsql . " LIMIT #,#) y ON ^posts.postid=y.postid";
*/

    // Zen Mogul FAST (with index on [created] field)
    ...
    $selectspec['source'] .=
		" WHERE " .
    // Zen : rule out categories for now?
	//	qa_db_categoryslugs_sql_args($categoryslugs, $selectspec['arguments']) .
	//	(isset($createip) ? "createip=UNHEX($) AND " : "") .
		$typesSQL . $sortsql . " LIMIT #,#";
...
//	$selectspec['source'] .= " JOIN (SELECT postid FROM ^posts WHERE " . qa_db_categoryslugs_sql_args($categoryslugs, $selectspec['arguments']) . "type=$ AND " . $bysql . " AND closedbyid IS NULL ORDER BY ^posts.created DESC LIMIT #,#) y ON ^posts.postid=y.postid";
	...
	$selectspec['source'] .= 
		" JOIN (SELECT postid FROM ^posts WHERE " . 
		qa_db_categoryslugs_sql_args($categoryslugs, $selectspec['arguments']) . $typesSQL . 
		" AND " . $bysql . " AND closedbyid IS NULL ORDER BY ^posts.created DESC LIMIT #,#) y ON ^posts.postid=y.postid";
...
function qa_db_recent_a_qs_selectspec($voteuserid, $start, $categoryslugs = null, $createip = null, $specialtype = false, $fullanswers = false, $count = null)
{
	...
	$selectspec['source'] .=
		...
		(FINAL_EXTERNAL_USERS ? "" : " LEFT JOIN ^users AS ausers ON aposts.userid=ausers.userid") .
...
//		($specialtype ? '' : " WHERE ^posts.type='Q'");
		($specialtype ? '' : " WHERE ^posts.type='Q' OR ^posts.type='POST'");
...
function qa_db_recent_c_qs_selectspec($voteuserid, $start, $categoryslugs = null, $createip = null, $specialtype = false, $fullcomments = false, $count = null)
{
	...
	$selectspec['source'] .=
		...
		(FINAL_EXTERNAL_USERS ? "" : " LEFT JOIN ^users AS cusers ON cposts.userid=cusers.userid") .
...
//		($specialtype ? '' : " WHERE ^posts.type='Q' AND ((parentposts.type='Q') OR (parentposts.type='A'))");
		($specialtype ? '' : " WHERE (^posts.type='Q' OR ^posts.type='POST') AND ((parentposts.type='Q' OR parentposts.type='POST') OR (parentposts.type='A'))");
...
function qa_db_recent_edit_qs_selectspec($voteuserid, $start, $categoryslugs = null, $lastip = null, $onlyvisible = true, $fulledited = false, $count = null)
{
	...
	$selectspec['source'] .=
		...
		(FINAL_EXTERNAL_USERS ? "" : " LEFT JOIN ^users AS editusers ON editposts.lastuserid=editusers.userid") .
...
//		($onlyvisible ? "type IN ('Q', 'A', 'C', 'POST')" : "1") .
...
//		($onlyvisible ? " WHERE parentposts.type IN ('Q', 'A', 'C') AND ^posts.type IN ('Q', 'A', 'C')" : "");
		($onlyvisible ? " WHERE parentposts.type IN ('Q', 'A', 'C', 'POST') AND ^posts.type IN ('Q', 'A', 'C', 'POST')" : "");
...
// Zen
//		" ^posts.postid=IF(LEFT(parentposts.type, 1)='Q', parentposts.postid, parentposts.parentid)" .
		" ^posts.postid=IF(LEFT(parentposts.type, 1)='Q' OR parentposts.type='POST', parentposts.postid, parentposts.parentid)" .
//		" JOIN ^posts AS flagposts ON parentposts.postid=IF(LEFT(flagposts.type, 1)='Q', flagposts.postid, flagposts.parentid)" .
		" JOIN ^posts AS flagposts ON parentposts.postid=IF(LEFT(flagposts.type, 1)='Q' OR parentposts.type='POST', flagposts.postid, flagposts.parentid)" .
		(FINAL_EXTERNAL_USERS ? "" : " LEFT JOIN ^users AS flagusers ON flagposts.userid=flagusers.userid") .
...
//		" JOIN (SELECT postid FROM ^posts WHERE flagcount>0 AND type IN ('Q', 'A', 'C') ORDER BY ^posts.flagcount DESC, ^posts.created DESC LIMIT #,#) y ON flagposts.postid=y.postid";
		" JOIN (SELECT postid FROM ^posts WHERE flagcount>0 AND type IN ('Q', 'A', 'C', 'POST') ORDER BY ^posts.flagcount DESC, ^posts.created DESC LIMIT #,#) y ON flagposts.postid=y.postid";
...
//		" JOIN ^posts AS childposts ON parentposts.postid=IF(LEFT(childposts.type, 1)='Q', childposts.postid, childposts.parentid)" .
		" JOIN ^posts AS childposts ON parentposts.postid=IF(LEFT(childposts.type, 1)='Q' OR parentposts.type='POST', childposts.postid, childposts.parentid)" .
...
//	$selectspec['source'] .= " WHERE ^posts.postid=(SELECT IF(LEFT(parent.type, 1)='A', parent.parentid, parent.postid) FROM ^posts AS child LEFT JOIN ^posts AS parent ON parent.postid=child.parentid WHERE child.postid=# AND parent.type IN('Q','A'))";
	$selectspec['source'] .= " WHERE ^posts.postid=(SELECT IF(LEFT(parent.type, 1)='A', parent.parentid, parent.postid) FROM ^posts AS child LEFT JOIN ^posts AS parent ON parent.postid=child.parentid WHERE child.postid=# AND parent.type IN('Q','A', 'POST'))";
...
*/
    ...
    $selectspec['source'] .= <<<SQL

    JOIN (
        SELECT postid, SUM(score)+LOG(postid)/1000000 AS score
        FROM
        (
--            (
--                SELECT ^titlewords.postid, LOG(#/titlecount) AS score FROM ^titlewords
--                JOIN ^words ON ^titlewords.wordid=^words.wordid
--                JOIN ^titlewords AS source ON ^titlewords.wordid=source.wordid
--                WHERE source.postid=# AND titlecount<#
--            )
--            UNION ALL
--            (
                SELECT ^posttags.postid, 2*LOG(#/tagcount) AS score
                FROM ^posttags
                JOIN ^words ON ^posttags.wordid=^words.wordid
                JOIN ^posttags AS source ON ^posttags.wordid=source.wordid
                WHERE source.postid=# AND tagcount<#

--            )
--            UNION ALL
--            (
--                SELECT ^posts.postid, LOG(#/^categories.qcount)
--                FROM ^posts
--                JOIN ^categories ON ^posts.categoryid=^categories.categoryid AND ^posts.type='Q'
--                WHERE ^categories.categoryid=(SELECT categoryid FROM ^posts WHERE postid=#) AND ^categories.qcount<#)
            ) x
            WHERE postid!=#
            GROUP BY postid
            ORDER BY score DESC
    ...
    ON ^posts.postid=y.postid;

SQL;

	array_push($selectspec['arguments'], IGNORED_WORDS_FREQ, $questionid, IGNORED_WORDS_FREQ, IGNORED_WORDS_FREQ,
		$questionid, IGNORED_WORDS_FREQ, IGNORED_WORDS_FREQ, $questionid, IGNORED_WORDS_FREQ, $questionid, $count);
...
//	$selectspec['source'] .= " JOIN (SELECT questionid, SUM(score)+2*(LOG(#)*(MAX(^posts.hotness)-(SELECT MIN(hotness) FROM ^posts WHERE type='Q'))/((SELECT MAX(hotness) FROM ^posts WHERE type='Q')-(SELECT MIN(hotness) FROM ^posts WHERE type='Q')))+LOG(questionid)/1000000 AS score, GROUP_CONCAT(CONCAT_WS(':', matchposttype, matchpostid, ROUND(score,3))) AS matchparts FROM (";
	$selectspec['source'] .= " JOIN (SELECT questionid, SUM(score)+2*(LOG(#)*(MAX(^posts.hotness)-(SELECT MIN(hotness) FROM ^posts WHERE type='Q' OR type='POST'))/((SELECT MAX(hotness) FROM ^posts WHERE type='Q')-(SELECT MIN(hotness) FROM ^posts WHERE type='Q')))+LOG(questionid)/1000000 AS score, GROUP_CONCAT(CONCAT_WS(':', matchposttype, matchpostid, ROUND(score,3))) AS matchparts FROM (";
	$selectspec['sortdesc'] = 'score';
	array_push($selectspec['arguments'], IGNORED_WORDS_FREQ);
	...
	if (!empty($titlewords)) {
		...
		array_push($selectspec['arguments'], IGNORED_WORDS_FREQ, $titlewords, IGNORED_WORDS_FREQ);
	}
	...
	if (!empty($contentwords)) {
		...
		$selectspec['source'] .= ($selectparts++ ? " UNION ALL " : "") .
			"(SELECT questionid, (1-1/(1+count))*LOG(#/contentcount)*(CASE ^contentwords.type WHEN ('Q' OR 'POST') THEN 1.0 WHEN 'A' THEN 0.5 ELSE 0.25 END) AS score, ^contentwords.type AS matchposttype, ^contentwords.postid AS matchpostid FROM ^contentwords JOIN ^words ON ^contentwords.wordid=^words.wordid WHERE word IN ($) AND contentcount<#)";

		array_push($selectspec['arguments'], IGNORED_WORDS_FREQ, $contentwords, IGNORED_WORDS_FREQ);
	}
	...
	if (!empty($tagwords)) {
		...
		array_push($selectspec['arguments'], IGNORED_WORDS_FREQ, $tagwords, IGNORED_WORDS_FREQ);
	}
	...
	if (!empty($handlewords)) {
		if (FINAL_EXTERNAL_USERS) {
			...
			if (count($userids)) {
				...
				array_push($selectspec['arguments'], IGNORED_WORDS_FREQ, $userids);
			}
			...
			array_push($selectspec['arguments'], IGNORED_WORDS_FREQ, $handlewords);
		}
	}
	...
	if (strlen($handle)) { // to allow searching for multi-word usernames (only works if search query contains full username and nothing else)
		if (FINAL_EXTERNAL_USERS) {
			...
			if (count($userids)) {
				...
				array_push($selectspec['arguments'], IGNORED_WORDS_FREQ, reset($userids));
			}
			...
			array_push($selectspec['arguments'], IGNORED_WORDS_FREQ, $handle);
		}
	}
...
function qa_db_category_nav_selectspec($slugsorid, $isid, $ispostid = false, $full = false)
{
	...
	$parentselects = array( // requiresCATEGORY_DEPTH=4
...
//	$selectspec['source'] .= " JOIN (SELECT postid FROM ^posttags WHERE wordid=(SELECT wordid FROM ^words WHERE word=$ AND word=$ COLLATE utf8_bin LIMIT 1) ORDER BY postcreated DESC LIMIT #,#) y ON ^posts.postid=y.postid";
	$selectspec['source'] .= " JOIN (SELECT postid FROM ^posttags WHERE wordid=(SELECT wordid FROM ^words WHERE LOWER(word)=$ LIMIT 1) ORDER BY postcreated DESC LIMIT #,#) y ON ^posts.postid=y.postid";
//	array_push($selectspec['arguments'], $tag, qa_strtolower($tag), $start, $count);
	array_push($selectspec['arguments'], qa_strtolower($tag), $start, $count);
	$selectspec['sortdesc'] = 'created';
...
function qa_db_tag_word_selectspec($tag)
{
	return array(
		...
		'source' => '^words WHERE LOWER(word)=$',
		'arguments' => array(qa_strtolower($tag)),
		'single' => true,
	);
...
// Zen
//	$selectspec['source'] .= " WHERE ^posts.userid=" . (FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)") . " AND type='Q' ORDER BY ^posts.created DESC LIMIT #,#";
	$selectspec['source'] .= " WHERE ^posts.userid=" . (FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)") . " AND (type='Q' OR type='POST') ORDER BY ^posts.created DESC LIMIT #,#";
...
function qa_db_user_recent_a_qs_selectspec($voteuserid, $identifier, $count = null, $start = 0)
{
	...
	$selectspec['source'] .=
		" JOIN ^posts AS aposts ON ^posts.postid=aposts.parentid" .
		" JOIN (SELECT postid FROM ^posts WHERE " .
		" userid=" . (FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)") .
...
//		" AND type='C' ORDER BY created DESC LIMIT #) y ON cposts.postid=y.postid WHERE ^posts.type='Q' AND parentposts.type IN ('Q', 'A')";
		" AND type='C' ORDER BY created DESC LIMIT #) y ON cposts.postid=y.postid WHERE (^posts.type='Q' OR ^posts.type='POST') AND parentposts.type IN ('Q', 'A', 'POST')";
...
//		" ^posts.postid=IF(LEFT(parentposts.type, 1)='Q', parentposts.postid, parentposts.parentid)" .
		" ^posts.postid=IF(LEFT(parentposts.type, 1)='Q' OR parentposts.type='POST', parentposts.postid, parentposts.parentid)" .
//		" JOIN ^posts AS editposts ON parentposts.postid=IF(LEFT(editposts.type, 1)='Q', editposts.postid, editposts.parentid)" .
		" JOIN ^posts AS editposts ON parentposts.postid=IF(LEFT(editposts.type, 1)='Q' OR editposts.type='POST', editposts.postid, editposts.parentid)" .
		" JOIN (SELECT postid FROM ^posts WHERE " .
		" lastuserid=" . (FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)") .
//		" AND type IN ('Q', 'A', 'C') ORDER BY updated DESC LIMIT #) y ON editposts.postid=y.postid " .
		" AND type IN ('Q', 'A', 'C', 'POST') ORDER BY updated DESC LIMIT #) y ON editposts.postid=y.postid " .
//		" WHERE parentposts.type IN ('Q', 'A', 'C') AND ^posts.type IN ('Q', 'A', 'C')";
		" WHERE parentposts.type IN ('Q', 'A', 'C', 'POST') AND ^posts.type IN ('Q', 'A', 'C', 'POST')";
...
{
	$count = isset($count) ? $count : DB_RETRIEVE_TAGS;
 ...
 * @return array
 */
function qa_db_user_points_selectspec($identifier, $isuserid = FINAL_EXTERNAL_USERS)
{
	return array(
 ...
 * @return array
 */
function qa_db_user_rank_selectspec($identifier, $isuserid = FINAL_EXTERNAL_USERS)
{
	return array(
...
{
	$count = isset($count) ? min($count, DB_RETRIEVE_USERS) : DB_RETRIEVE_USERS;
...
function qa_db_users_with_flag_selectspec($flag, $start = 0, $limit = null)
{
	...
	if (isset($limit)) {
		$limit = min($limit, DB_RETRIEVE_USERS);
		$source .= ' LIMIT #,#';
...
{
	$count = isset($count) ? min($count, DB_RETRIEVE_MESSAGES) : DB_RETRIEVE_MESSAGES;
...
function qa_db_messages_inbox_selectspec($type, $toidentifier, $toisuserid, $start = 0, $limit = null)
{
	...
	if (isset($limit)) {
		$limit = min($limit, DB_RETRIEVE_MESSAGES);
		$source .= ' LIMIT #,#';
		$arguments[] = $start;
		$arguments[] = $limit;
	}
...
//	$selectspec['source'] .= ' JOIN ^userfavorites AS selectfave ON ^posts.postid=selectfave.entityid WHERE selectfave.userid=$ AND selectfave.entitytype=$ AND ^posts.type="Q" ORDER BY ^posts.created DESC';
	$selectspec['source'] .= ' JOIN ^userfavorites AS selectfave ON ^posts.postid=selectfave.entityid WHERE selectfave.userid=$ AND selectfave.entitytype=$ AND (^posts.type="Q" OR ^posts.type="POST") ORDER BY ^posts.created DESC';
	$selectspec['arguments'][] = $userid;
	$selectspec['arguments'][] = ENTITY_QUESTION;

	if (isset($limit)) {
		$limit = min($limit, DB_RETRIEVE_QS_AS);
...
function qa_db_user_favorite_users_selectspec($userid, $limit = null, $start = 0)
{
	...
	$arguments = array($userid, ENTITY_USER);
...
function qa_db_user_favorite_tags_selectspec($userid, $limit = null, $start = 0)
{
	...
	$arguments = array($userid, ENTITY_TAG);

	if (isset($limit)) {
		$limit = min($limit, DB_RETRIEVE_TAGS);
		$source .= ' LIMIT #,#';
		$arguments[] = $start;
		$arguments[] = $limit;
	}
...
function qa_db_user_favorite_categories_selectspec($userid)
{
	...
	return array(
		...
		'arguments' => array($userid, ENTITY_CATEGORY),
		'sortasc' => 'title',
	);
}
...
function qa_db_user_favorite_non_qs_selectspec($userid)
{
	...
	return array(
		...
		'arguments' => array(ENTITY_USER, ENTITY_TAG, ENTITY_CATEGORY, $userid, ENTITY_QUESTION),
	);
}
...
function qa_db_user_updates_selectspec($userid, $forfavorites = true, $forcontent = true)
{
	...
	$nonesql = qa_db_argument_to_mysql(ENTITY_NONE, true);
...
//		" AND (updateposts.type IN ('Q', 'A', 'C') OR fullevents.entitytype=" . $nonesql . ")" .
		" AND (updateposts.type IN ('Q', 'A', 'C', 'POST') OR fullevents.entitytype=" . $nonesql . ")" .
//		" AND (^posts.selchildid=fullevents.lastpostid OR NOT fullevents.updatetype<=>$) AND ^posts.type IN ('Q', 'Q_HIDDEN')" .
		" AND (^posts.selchildid=fullevents.lastpostid OR NOT fullevents.updatetype<=>$) AND ^posts.type IN ('Q', 'Q_HIDDEN', 'POST')" .
		(FINAL_EXTERNAL_USERS ? '' : ' LEFT JOIN ^users AS eventusers ON fullevents.lastuserid=eventusers.userid') .
		' LEFT JOIN ^userpoints AS eventuserpoints ON fullevents.lastuserid=eventuserpoints.userid';
	$selectspec['arguments'][] = UPDATE_SELECTED;
 ...
 * @return array
 */
function qa_db_user_levels_selectspec($identifier, $isuserid = FINAL_EXTERNAL_USERS, $full = false)
{
	require_once INCLUDE_DIR . 'app/updates.php';
	...
	if ($full) {
		...
		array_unshift($selectspec['arguments'], ENTITY_CATEGORY);
	}
