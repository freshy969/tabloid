...
/*
	Database access functions which are specific to the admin center
*/
...
{
	if (defined('USERS_PREFIX')) { // check if one of the prefixes is a prefix itself of the other
		if (stripos(USERS_PREFIX, TABLE_PREFIX) === 0)
			$prefixes = array(TABLE_PREFIX);
		elseif (stripos(TABLE_PREFIX, USERS_PREFIX) === 0)
			$prefixes = array(USERS_PREFIX);
		else
			$prefixes = array(TABLE_PREFIX, USERS_PREFIX);
...
function qa_db_get_unapproved_users($count)
{
	$results = qa_db_read_all_assoc(qa_db_query_sub(
		...
		USER_LEVEL_APPROVED, USER_FLAGS_USER_BLOCKED, $count
	));
...
function qa_db_category_child_depth($categoryid)
{
	...
	$result = qa_db_read_one_assoc(qa_db_query_sub(
		'SELECT COUNT(child1.categoryid) AS count1, COUNT(child2.categoryid) AS count2, COUNT(child3.categoryid) AS count3 FROM ^categories AS child1 LEFT JOIN ^categories AS child2 ON child2.parentid=child1.categoryid LEFT JOIN ^categories AS child3 ON child3.parentid=child2.categoryid WHERE child1.parentid=#;', // requiresCATEGORY_DEPTH=4
		$categoryid
	));

	for ($depth = CATEGORY_DEPTH - 1; $depth >= 1; $depth--)
...
function qa_db_categories_recalc_backpaths($firstcategoryid, $lastcategoryid = null)
{
	...
	qa_db_query_sub(
		...
		$firstcategoryid, $lastcategoryid // requiresCATEGORY_DEPTH=4
	);
}
