...
/*
	Database-level access to user management tables (if not using single sign-on)
*/
...
{
	if (qa_should_update_counts() && !FINAL_EXTERNAL_USERS) {
		qa_db_query_sub(
			"INSERT INTO ^options (title, content) " .
			"SELECT 'cache_uapprovecount', COUNT(*) FROM ^users " .
			"WHERE level < # AND NOT (flags & #) " .
			"ON DUPLICATE KEY UPDATE content = VALUES(content)",
			USER_LEVEL_APPROVED, USER_FLAGS_USER_BLOCKED
		);
	}
}
