...
/*
	Database functions for recalculations (clean-up operations)
*/
...
function qa_db_posts_get_for_reindexing($startpostid, $count)
{
	return qa_db_read_all_assoc(qa_db_query_sub(			
		...
		"SELECT ^posts.postid, ^posts.title, ^posts.content, ^posts.format, ^posts.tags, ^posts.categoryid, ^posts.type, ".
		"IF (^posts.type='Q', ^posts.postid, IF(parent.type='Q', parent.postid, grandparent.postid)) AS questionid, ^posts.parentid ".
		"FROM ^posts ".
		...
		"WHERE ^posts.postid >= # AND ( (^posts.type = 'Q' OR ^posts.type = 'POST') OR (^posts.type = 'A' AND parent.type <=> 'Q') OR (^posts.type = 'C' AND parent.type <=> 'Q') OR (^posts.type = 'C' AND parent.type <=> 'A' AND grandparent.type <=> 'Q') ) ".
...
function qa_db_posts_votes_recount($firstpostid, $lastpostid)
{
	qa_db_query_sub(
		...
		'UPDATE ^posts AS x, (SELECT ^posts.postid, COALESCE(SUM(GREATEST(0,^uservotes.vote)),0) AS upvotes,'.
		'-COALESCE(SUM(LEAST(0,^uservotes.vote)),0) AS downvotes,'.
		'COALESCE(SUM(IF(^uservotes.flag, 1, 0)),0) AS flagcount '.
		'FROM ^posts '.
		'LEFT JOIN ^uservotes ON ^uservotes.postid = ^posts.postid '.
		'WHERE ^posts.postid >= # AND ^posts.postid <= # '.
		'GROUP BY postid) AS a '.
		'SET x.upvotes = x.extvotes + a.upvotes, '.
		'x.downvotes = a.downvotes, '.
		'x.netvotes = x.extvotes + a.upvotes - a.downvotes, '.
		'x.flagcount = a.flagcount '.
...
function qa_db_posts_answers_recount($firstpostid, $lastpostid)
{
	...
	qa_db_query_sub(
		...
		'UPDATE ^posts AS x, '.
		'(SELECT parents.postid, COUNT(children.postid) AS acount, '.
		'COALESCE(GREATEST(MAX(children.netvotes), 0), 0) AS amaxvote '.
		'FROM ^posts AS parents '.
		'LEFT JOIN ^posts AS children ON parents.postid = children.parentid AND children.type = \'A\' '.
		'WHERE parents.postid >= # AND parents.postid <= # '.
		'GROUP BY postid) AS a '.
		'SET x.acount = a.acount, '.
		'x.amaxvote = a.amaxvote '.
		'WHERE x.postid = a.postid',
