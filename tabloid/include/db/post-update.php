...
/*
	Database functions for changing a question, answer or comment
*/
...
function qa_db_post_set_selchildid($questionid, $selchildid, $lastuserid = null, $lastip = null)
{
	...
	if (isset($selchildid) && isset($lastuserid) && isset($lastip)) {
		qa_db_query_sub(
			...
			UPDATE_SELECTED, $lastuserid, bin2hex(@inet_pton($lastip)), $selchildid
		);
	}
}
...
function qa_db_post_set_closed($questionid, $closedbyid, $lastuserid = null, $lastip = null)
{
	if (isset($lastuserid) || isset($lastip)) {
		qa_db_query_sub(
			...
			$closedbyid, UPDATE_CLOSED, $lastuserid, bin2hex(@inet_pton($lastip)), $questionid
		);
	} else {
		qa_db_query_sub(
 ...
 */
function qa_db_post_set_type($postid, $type, $lastuserid = null, $lastip = null, $updatetype = UPDATE_TYPE)
{
	if (isset($lastuserid) || isset($lastip)) {
		qa_db_query_sub(
...
function qa_db_post_set_parent($postid, $parentid, $lastuserid = null, $lastip = null)
{
	if (isset($lastuserid) || isset($lastip)) {
		qa_db_query_sub(
			...
			$parentid, UPDATE_PARENT, $lastuserid, bin2hex(@inet_pton($lastip)), $postid
		);
	} else {
		qa_db_query_sub(
 ...
 * @param $name
 */
function qa_db_post_set_content($postid, $title, $content, $format, $tagstring, $notify, $lastuserid = null, $lastip = null, $updatetype = UPDATE_CONTENT, $name = null,
                                $type = null, $image = null, $lead = null) 
{
	...
	if (isset($lastuserid) || isset($lastip)) {
		...
		if ($type) {
	        qa_db_query_sub(
    	        'UPDATE ^posts SET title=$, content=$, format=$, tags=$, name=COALESCE($, name), notify=$, updated=NOW(), updatetype=$, lastuserid=$, lastip=UNHEX($), type=$, image=$, `lead`=$ WHERE postid=#',
        	    $title, $content, $format, $tagstring, $name, $notify, $updatetype, $lastuserid, bin2hex(@inet_pton($lastip)), $type, $image, $lead, $postid
	        );
		} else {
	        qa_db_query_sub(
		...
		qa_db_query_sub(
			'UPDATE ^posts SET title=$, content=$, format=$, tags=$, name=COALESCE($, name), notify=$, type=$, image=$, `lead`=$ WHERE postid=#',
			$title, $content, $format, $tagstring, $name, $notify, $type, $image, $lead, $postid
		);
...
function qa_db_post_set_category($postid, $categoryid, $lastuserid = null, $lastip = null)
{
	if (isset($lastuserid) || isset($lastip)) {
		qa_db_query_sub(
			...
			$categoryid, UPDATE_CATEGORY, $lastuserid, bin2hex(@inet_pton($lastip)), $postid
		);
	} else {
		qa_db_query_sub(
