...
/*
	Database-level functions for installation and upgrading
*/

define('DB_VERSION_CURRENT', 67);
	...
	*/
	...
	$tables = array(
		'users' => array(
			...
			'email' => 'VARCHAR(' . DB_MAX_EMAIL_LENGTH . ') NOT NULL',
			'handle' => 'VARCHAR(' . DB_MAX_HANDLE_LENGTH . ') NOT NULL', // username
		...
		'userprofile' => array(
			'userid' => $useridcoltype . ' NOT NULL',
			'title' => 'VARCHAR(' . DB_MAX_PROFILE_TITLE_LENGTH . ') NOT NULL', // profile field name
			'content' => 'VARCHAR(' . DB_MAX_PROFILE_CONTENT_LENGTH . ') NOT NULL', // profile field value
			'UNIQUE userid (userid,title)',
		),

		'userfields' => array(
			'fieldid' => 'SMALLINT UNSIGNED NOT NULL AUTO_INCREMENT',
			'title' => 'VARCHAR(' . DB_MAX_PROFILE_TITLE_LENGTH . ') NOT NULL', // to match title column in userprofile table
			'content' => 'VARCHAR(' . DB_MAX_PROFILE_TITLE_LENGTH . ')', // label for display on user profile pages - NULL means use default
			'position' => 'SMALLINT UNSIGNED NOT NULL',
			'flags' => 'TINYINT UNSIGNED NOT NULL', //FIELD_FLAGS_* at top of /qa-include/app/users.php
			'permit' => 'TINYINT UNSIGNED', // minimum user level required to view (usesPERMIT_* constants), null means no restriction
			'PRIMARY KEY (fieldid)',
		),
		...
		'messages' => array(
			...
			'content' => 'VARCHAR(' . DB_MAX_CONTENT_LENGTH . ') NOT NULL',
			'format' => 'VARCHAR(' . DB_MAX_FORMAT_LENGTH . ') CHARACTER SET ascii NOT NULL',
			'created' => 'DATETIME NOT NULL',
		...
		'usernotices' => array(
			...
			'content' => 'VARCHAR(' . DB_MAX_CONTENT_LENGTH . ') NOT NULL',
			'format' => 'VARCHAR(' . DB_MAX_FORMAT_LENGTH . ') CHARACTER SET ascii NOT NULL',
			'tags' => 'VARCHAR(' . DB_MAX_CAT_PAGE_TAGS_LENGTH . ')', // any additional information for a plugin to access
			'created' => 'DATETIME NOT NULL',
		...
		'categories' => array(
			...
			'title' => 'VARCHAR(' . DB_MAX_CAT_PAGE_TITLE_LENGTH . ') NOT NULL', // category name
			'tags' => 'VARCHAR(' . DB_MAX_CAT_PAGE_TAGS_LENGTH . ') NOT NULL', // slug (url fragment) used to identify category
			'content' => 'VARCHAR(' . DB_MAX_CAT_CONTENT_LENGTH . ') NOT NULL DEFAULT \'\'', // description of category
			...
			'backpath' => 'VARCHAR(' . (CATEGORY_DEPTH * (DB_MAX_CAT_PAGE_TAGS_LENGTH + 1)) . ') NOT NULL DEFAULT \'\'',
			'PRIMARY KEY (categoryid)',
			'UNIQUE parentid (parentid, tags)',
			'UNIQUE parentid_2 (parentid, position)',
			'KEY backpath (backpath(' . DB_MAX_CAT_PAGE_TAGS_LENGTH . '))',
		),

		'pages' => array(
			'pageid' => 'SMALLINT UNSIGNED NOT NULL AUTO_INCREMENT',
			'title' => 'VARCHAR(' . DB_MAX_CAT_PAGE_TITLE_LENGTH . ') NOT NULL', // title for navigation
			...
			'permit' => 'TINYINT UNSIGNED', // is there a minimum user level required for it (usesPERMIT_* constants), null means no restriction
			'tags' => 'VARCHAR(' . DB_MAX_CAT_PAGE_TAGS_LENGTH . ') NOT NULL', // slug (url fragment) for page, or url for external pages
			'heading' => 'VARCHAR(' . DB_MAX_TITLE_LENGTH . ')', // for display within <h1> tags
		...
		'widgets' => array(
			...
			'tags' => 'VARCHAR(' . DB_MAX_WIDGET_TAGS_LENGTH . ') CHARACTER SET ascii NOT NULL', // comma-separated list of templates to display on
			'title' => 'VARCHAR(' . DB_MAX_WIDGET_TITLE_LENGTH . ') NOT NULL', // name of widget module that should be displayed
		...
		'posts' => array(
			...
			'catidpath2' => 'INT UNSIGNED', // note thatCATEGORY_DEPTH=4
			...
			'format' => 'VARCHAR(' . DB_MAX_FORMAT_LENGTH . ') CHARACTER SET ascii NOT NULL DEFAULT \'\'', // format of content, e.g. 'html'
			'created' => 'DATETIME NOT NULL',
			'updated' => 'DATETIME', // time of last update
			'updatetype' => 'CHAR(1) CHARACTER SET ascii', // see /qa-include/app/updates.php
			'title' => 'VARCHAR(' . DB_MAX_TITLE_LENGTH . ')',
			'content' => 'VARCHAR(' . DB_MAX_CONTENT_LENGTH . ')',
			'tags' => 'VARCHAR(' . DB_MAX_TAGS_LENGTH . ')', // string of tags separated by commas
			'name' => 'VARCHAR(' . DB_MAX_NAME_LENGTH . ')', // name of author if post anonymonus
			'notify' => 'VARCHAR(' . DB_MAX_EMAIL_LENGTH . ')', // email address, or @ to get from user, or NULL for none
			'PRIMARY KEY (postid)',
			...
			'KEY catidpath2 (catidpath2, type, created)', // note thatCATEGORY_DEPTH=4
			...
			'KEY catidpath2_2 (catidpath2, updated, type)', // note thatCATEGORY_DEPTH=4
		...
		'blobs' => array(
			...
			'format' => 'VARCHAR(' . DB_MAX_FORMAT_LENGTH . ') CHARACTER SET ascii NOT NULL', // format e.g. 'jpeg', 'gif', 'png'
			'content' => 'MEDIUMBLOB', // null means it's stored on disk in BLOBS_DIRECTORY
			'filename' => 'VARCHAR(' . DB_MAX_BLOB_FILE_NAME_LENGTH . ')', // name of source file (if appropriate)
		...
		'words' => array(
			...
			'word' => 'VARCHAR(' . DB_MAX_WORD_LENGTH . ') NOT NULL',
		...
		'options' => array(
			'title' => 'VARCHAR(' . DB_MAX_OPTION_TITLE_LENGTH . ') NOT NULL', // name of option
			'content' => 'VARCHAR(' . DB_MAX_CONTENT_LENGTH . ') NOT NULL', // value of option
			'PRIMARY KEY (title)',
		),
		...
		'usermetas' => array(
			'userid' => $useridcoltype . ' NOT NULL',
			'title' => 'VARCHAR(' . DB_MAX_META_TITLE_LENGTH . ') NOT NULL',
			'content' => 'VARCHAR(' . DB_MAX_META_CONTENT_LENGTH . ') NOT NULL',
		...
		'tagmetas' => array(
			'tag' => 'VARCHAR(' . DB_MAX_WORD_LENGTH . ') NOT NULL',
			'title' => 'VARCHAR(' . DB_MAX_META_TITLE_LENGTH . ') NOT NULL',
			'content' => 'VARCHAR(' . DB_MAX_META_CONTENT_LENGTH . ') NOT NULL',
...
{
	require_once INCLUDE_DIR . 'db/options.php';
...
function qa_db_check_tables()
{
	...
	if (count($missing) == count($definitions))
		...
		if (!isset($missing['options'])) {
			...
			if (isset($version) && ($version < DB_VERSION_CURRENT))
				return 'old-version';
		}

		if (count($missing)) {
			if (defined('USERS_PREFIX')) { // special case if two installations sharing users
				$datacount = 0;
				$datamissing = 0;

				foreach ($definitions as $rawname => $definition) {
					if (qa_db_add_table_prefix($rawname) == (TABLE_PREFIX . $rawname)) {
						$datacount++;
...
function qa_db_install_tables()
{
	...
	qa_db_set_db_version(DB_VERSION_CURRENT);
}
...
function qa_db_default_userfields_sql()
{
	...
	$profileFields = array(
		...
		array(
			'title' => 'website',
			'position' => 3,
			'flags' => FIELD_FLAGS_LINK_URL,
			'permit' =>PERMIT_ALL,
		),
		array(
			'title' => 'about',
			'position' => 4,
			'flags' => FIELD_FLAGS_MULTI_LINE,
			'permit' =>PERMIT_ALL,
		),
	);
...
function qa_db_upgrade_tables()
{
	...
	while (1) {
		...
		qa_db_upgrade_progress(DB_VERSION_CURRENT - $version . ' upgrade step/s remaining...');
		...
		switch ($newversion) {
			...
			case 30:
				...
				qa_db_upgrade_query('ALTER TABLE ^categories CHANGE COLUMN categoryid categoryid ' . $definitions['categories']['categoryid'] . ', ADD COLUMN parentid ' . $definitions['categories']['parentid'] . ' AFTER categoryid, ADD COLUMN backpath ' . $definitions['categories']['backpath'] . ' AFTER position, ADD COLUMN content ' . $definitions['categories']['content'] . ' AFTER tags, DROP INDEX tags, DROP INDEX position, ADD UNIQUE parentid (parentid, tags), ADD UNIQUE parentid_2 (parentid, position), ADD KEY backpath (backpath(' . DB_MAX_CAT_PAGE_TAGS_LENGTH . '))');
				qa_db_upgrade_query($locktablesquery);
			...
			case 35:
				...
				qa_db_upgrade_query(qa_db_create_table_sql('userfavorites', array(
					...
					'KEY userid (userid, nouserevents)',
					'KEY entitytype (entitytype, entityid, nouserevents)',
					FINAL_EXTERNAL_USERS ? null : 'CONSTRAINT ^userfavorites_ibfk_1 FOREIGN KEY (userid) REFERENCES ^users(userid) ON DELETE CASCADE',
				)));
			...
			case 36:
				...
				qa_db_upgrade_query(qa_db_create_table_sql('userevents', array(
					...
					FINAL_EXTERNAL_USERS ? null : 'CONSTRAINT ^userevents_ibfk_1 FOREIGN KEY (userid) REFERENCES ^users(userid) ON DELETE CASCADE',
				)));
			...
			case 42:
				...
				qa_db_upgrade_query(qa_db_create_table_sql('usermetas', array(
					...
					'PRIMARY KEY (userid, title)',
					FINAL_EXTERNAL_USERS ? null : 'CONSTRAINT ^usermetas_ibfk_1 FOREIGN KEY (userid) REFERENCES ^users(userid) ON DELETE CASCADE',
				)));
			...
			case 47:
				...
				qa_db_upgrade_query(qa_db_create_table_sql('usernotices', array(
					...
					'PRIMARY KEY (noticeid)',
					'KEY userid (userid, created)',
					FINAL_EXTERNAL_USERS ? null : 'CONSTRAINT ^usernotices_ibfk_1 FOREIGN KEY (userid) REFERENCES ^users(userid) ON DELETE CASCADE',
				)));
			...
			case 54:
				...
				qa_db_upgrade_query(qa_db_create_table_sql('userlevels', array(
					...
					'UNIQUE userid (userid, entitytype, entityid)',
					'KEY entitytype (entitytype, entityid)',
					FINAL_EXTERNAL_USERS ? null : 'CONSTRAINT ^userlevels_ibfk_1 FOREIGN KEY (userid) REFERENCES ^users(userid) ON DELETE CASCADE',
				)));
			...
			case 61:
				...
				$newlength = DB_MAX_CONTENT_LENGTH;
				...
				$oldlength = qa_db_read_one_value(qa_db_query_sub($query, DB_NAME, $tablename));
