...
/*
	Page module class for WYSIWYG editor (CKEditor) file upload receiver
*/


class wysiwyg_upload
{
		...
		if (is_array($_FILES) && count($_FILES)) {
			if (qa_opt('wysiwyg_editor_upload_images')) {
				...
				$upload = qa_upload_file_one(
					...
					$onlyImage ? MAX_IMAGE_WIDTH : null, // max width if it's an image upload
					null // no max height
				);
